...
#' \code{EXTdec}{ = 0.5, turns on changes in O2 extraction efficiency to simulate panting, the value being a coefficient determing the rate of an exponential decline from EXTMAX to the EXTmin}\cr\cr
#'
...
endoR <- function(
  ...
  EXTdec = 0.5, # turns on changes in O2 extraction efficiency to simulate panting, the value being a coefficient determing the rate of an exponential decline from EXTMAX to the EXTmin
    ...
    if(GMULT < GMULTMAX){
      ...
      if(AK1 < AKMAX){
        ...
        if(TC < TCMAX){
          ...
          if(EXTREF > EXTMIN){
            EXTSTEP <- EXTSTEP + 1
            EXTREF <- round(EXTMIN - (EXTMIN - EXTMAX) * exp(-EXTdec * EXTSTEP), 1)
          }else{
            EXTREF <- EXTMIN
            SKINW <- SKINW + SWEAT
