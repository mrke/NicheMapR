...
#' @param elevation = 0, elevation (m), needed for properties of air
...
#' P_e <- soil.hydro$air_entry_water_potential # J/kg
#' b <- soil.hydro$campbell_b_parameter # -
#' K_sat <- soil.hydro$saturated_hydraulic_conductivity # kg s/m3
#'
#' t <- seq(0, 24 * 3600 * 150, 24 * 3600) # seconds
#'
#' elevation <- 0 # m, need elevation for air properties
...
#'
#' # input parameters
#' indata <- list(shape = shape, K_e = K_e, spec_hyd = spec_hyd, f_air = f_air, pct_wet = pct_wet, elevation = elevation, vel = vel, K_sat = K_sat, P_e = P_e, b = b)
#'
#' # solve
...
egg_water <- function(t, y, indata) {
  with(as.list(c(indata, y)), {
    ...
    egg_evap <- function(T_air, T_egg, RH, vel, elevation, m, A_e, pct_wet){
      # constants
      ...
      # air properties
      dryair <- DRYAIR(db = T_air, alt = elevation)
    ...
    if(RH > 100){RH <- 100}
    if(RH < 0){RH <- 0}
    m_a <- egg_evap(T_air, T_egg, RH, vel, elevation, m, A_e, pct_wet) # kg/s, evaporative water exchange
