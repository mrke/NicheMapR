...
#' @param Tairf air temperature function with time, generated by 'approxfun' (deg C)
#' @param Tradf radiant temperature function with time, generated by 'approxfun'(deg C), averaging ground and sky
...
#' @usage ode(y = c(Tc_init, Ts_init, To_init), times = time, func = twolump, parms = indata)
#' @examples
...
#'
#'   with(Tbs_ode, plot(Tc ~ time, type = 'l', col = '1', ylim = c(-10, 80), xlim = c(0, 23), ylab='Temperature, deg C',xlab = 'hour of day', main = paste0(loc, ", ", mons[i], ", ", Ww_g," g")))
...
twolump<-function(t,y,indata){
  with(as.list(c(indata, y)), {
    ...
    Ts <- as.numeric(y[2]) # surface temperature
    To <- as.numeric(y[3]) # skin temperature
    ...
    if (posture == 'p') {
      Qalpha <- (Qnorm * (1 - pctdif) * ASILP + Qsol * pctdif * fatosk * ATOT + Qsol * (1 - alpha_sub) * fatosb * ATOT) * alpha
    }
    if (posture == 'n') {
      Qalpha <- (Qnorm * (1 - pctdif) * ASILN + Qsol * pctdif * fatosk * ATOT + Qsol * (1 - alpha_sub) * fatosb * ATOT) * alpha
    }
    if (posture == 'b') {
      Qalpha <- (Qnorm * (1 - pctdif) * (ASILN + ASILP) / 2 + Qsol * pctdif * fatosk * ATOT + Qsol * (1 - alpha_sub) * fatosb * ATOT) * alpha
    }
    ...
    beta <- F * To + (H * B - F * D) / (B - A) # from eq 133
    ...
    dTc <- (Qgen - Qresp - (Tc - To) / Rb) / Cc # from eq 103
    dTs <- ((Tc - To) / Rb - (Ts - To) / (Rs/2)) / Cs
    To <- (Qabs + Trad / Rrad + Tair / Rconv + 2 * Ts / Rs) / (1 / Rrad + 1 / Rconv + 2 / Rs) #-A * To + B * Tc + D#((Tc - Ts) / Rb - (To - Ts) / (Rs / 2)) / Cs # from eq 104
    #Tc <- Tc + dTc
    #Ts <- Ts + dTs
    #Tc
    #Ts
