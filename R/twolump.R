...
#' @param Ts_init = 5.1, initial shell temperature (°C) 
#' @param To_init = 5.2, initial surface temperature (°C) 
...
#' @return Ts 'Skin' / shell temperature (°C)
#' @return To Outer temperature (°C)
...
#'   To_init <- Tc_init
#'
#'   Tbs_ode <- as.data.frame(ode(y = c(Tc_init, Ts_init, To_init), times = time, func = onelump_var, parms = indata))
...
#twolump2<-function(t = seq(1, 3600, 60), Tc_init = 5, Ts_init = 5.1, To_init = 5.2, mass = 500, x_shell = 0.001,
  ...
  To <- as.numeric(y[3])#as.numeric(y[3]) # skin temperature
