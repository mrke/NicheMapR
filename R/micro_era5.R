...
#' @param weather.elevation optional value indicating the elevation of values in `hourlydata`. Either a numeric value, corresponding to the elevation in (m) of the location from which `hourlydata` were obtained, or `era5` (default, derived from Copernicus ERA5 climate reanalysis).
...
micro_era5 <- function(
  ...
  # --- 1. Input validation ---
  errors <- 0
  reference_height <- 2 # Reference height (m): ERA5 data is at 2 m screen height
  ...
  if(errors==0){ # continue
    ...
    if(save != 2){
      ...
      microclima.out <- microclima::microclimaforNMR(lat = longlat[2],
                                                     ...
                                                     dstart = dstart,
                                                     dfinish = dfinish,
                                                     l = mean(microclima.leaf_area_index),
                                                     x = LOR,
                                                     coastal = coastal,
                                                     ...
                                                     reanalysis2 = TRUE,
                                                     difani = FALSE,
                                                     weather.elevation = weather.elevation,
