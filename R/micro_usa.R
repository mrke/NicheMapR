...
micro_usa <- function(
  ...
  # --- 1. Input validation ---
  reference_height <- 2  # GRIDMET met data reference height (m) — 2 m standard screen height
  errors <- 0
  # usa-specific checks
  if(is.numeric(loc[1])){
  ...
  if(errors==0){ # continue
    ...
    if(save != 2){
      ...
      if(opendap == 1){
        ...
        nc <- RNetCDF::open.nc(paste0(baseurl, "/elevation/metdata_elevationdata.nc"))
    ...
    if(is.na(ALTITUDES)!=TRUE){
      ...
      reference_wind_max <- Wind * wind_multiplier
      reference_wind_min <- Wind * wind_multiplier
