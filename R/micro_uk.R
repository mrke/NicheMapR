...
+ellps=airy +datum=OSGB36 +units=m +no_defs'
    ...
    for(j in 1:nyears){ # start loop through years
      for(jj in 1:12){ # start loop through months
        cat(paste('reading weather input for ',yearlist[j],' month ', month[jj], ' \n',sep=""))
        # daily temperature range, K
        nc<-nc_open(paste(spatial,"/chess_dtr_",yearlist[j],month[jj],'.nc',sep=""))
        ...
        nc_close(nc)
        # daily precipitation, kg/m3/s
        nc<-nc_open(paste(spatial,"/chess_precip_",yearlist[j],month[jj],'.nc',sep=""))
        precip<-as.numeric(ncvar_get(nc,varid="precip",start=start,count))
        nc_close(nc)
        # daily solar radiation, W/m2 (averaged over day)
        nc<-nc_open(paste(spatial,"/chess_rsds_",yearlist[j],month[jj],'.nc',sep=""))
        rsds<-as.numeric(ncvar_get(nc,varid="rsds",start=start,count))
        nc_close(nc)
        # daily wind speed, m/s
        nc<-nc_open(paste(spatial,"/chess_sfcWind_",yearlist[j],month[jj],'.nc',sep=""))
        sfcWind<-as.numeric(ncvar_get(nc,varid="sfcWind",start=start,count))
        nc_close(nc)
        # compute final values
        if(j==1 & jj ==1){ # first month
          Tmax <- tas - 273.15 + dtr/2 # maximum air temperature, deg C
          Tmin <- tas - 273.15 - dtr/2 # minimum air temperature, deg C
          ...
          Wind <- c(Wind, sfcWind) # wind speed, m/s
