...
+ellps=airy +datum=OSGB36 +units=m +no_defs'
    ...
    if(is.na(ALTITUDES)!=TRUE){
      ...
      dates <- seq(as.POSIXct(paste0("01/01/",ystart), format = "%d/%m/%Y", tz = 'GMT'), as.POSIXct(paste0("01/01/",yfinish), format = "%d/%m/%Y", tz = 'GMT'), by = 'hours')
      dates2 <- seq(as.POSIXct(paste0("01/01/",ystart), format = "%d/%m/%Y", tz = 'GMT'), as.POSIXct(paste0("01/01/",yfinish), format = "%d/%m/%Y", tz = 'GMT'), by = 'days')
      if(lamb == 1){
