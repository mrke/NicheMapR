...
micro_aust_forecast <- function(loc="Nyrripi, Northern Territory",timeinterval=365,ystart=1990,yfinish=1990,
  ...
  rainfrac=0.5,
  shore=0,tides=matrix(data = 0., nrow = 24*timeinterval*nyears, ncol = 3),loop=0, scenario="",barcoo="",quadrangle=1, hourly=1, rainhourly = 0, getdata=1, lamb = 0, IUV = 0) {
  #
...
#   ZH2=0
  ...
  if(errors==0){ # continue
    ...
    julday<-c(forecast[1,2],forecast[1,2]+1,forecast[1,2]+2) # middle day of each month
    juldaysn2<-julday
    juldaysn<-julday
