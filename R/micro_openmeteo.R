...
micro_openmeteo <- function(
  ...
  if(errors==0){ # continue
    ...
    elev <- as.numeric(terra::extract(dem, vect(x, crs="+proj=longlat +datum=WGS84")))[2]
    if(save != 2){
      if(soilgrids == 1){
    ...
    tt <- seq(as.POSIXct(dstart2, format = "%d/%m/%Y", tz = 'UTC'), as.POSIXct(dfinish, format = "%d/%m/%Y", tz = 'UTC')+23*3600, by = 'hours')
    ...
    if(save != 2){
      ...
      ALTT <- elev
      TAIRhr <- openmeteo.out$hourly_temperature_2m
      if(scenario != 0){
        TAIRhr_orig <- TAIRhr
      ...
      SOLRhr <- openmeteo.out$hourly_shortwave_radiation
      if(scenario != 0){
      ...
      IRDhr <- openmeteo.out$hourly_temperature_2m
      IRDhr <- IRDhr * 0 + -1 # make negative so computed internally in the microclimate model
      CLDhr <- openmeteo.out$hourly_cloudcover
      if(scenario != 0){
        CLDhr <- CLDhr * (1 + 1 - SOLAR_diff)
      }
      CLDhr[CLDhr < 0] <- 0
      CLDhr[CLDhr > 100] <- 100
      RHhr <- openmeteo.out$hourly_relative_humidity_2m
      RHhr[RHhr > 100] <- 100
      RHhr[RHhr < 0] <- 0
      if(scenario != 0){
      ...
      WNhr <- openmeteo.out$hourly_windspeed_10m * 1000 / 3600
      WNhr[is.na(WNhr)] <- 0.1
      WNhr <- WNhr * (2 / 10) ^ 0.15 # adjust to 2m
      if(rainhourly == 0){
        RAINhr = rep(0, 24 * ndays)
      }else{
        RAINhr = openmeteo.out$hourly_precipitation
      }
      ...
      tt <- openmeteo.out$datetime
      jd <- julday(as.numeric(format(tt, "%Y")), as.numeric(format(tt, "%m")), as.numeric(format(tt, "%d")))
      alt.angle <- solalt(as.numeric(format(tt[1:24], '%H')), x[2], x[1], jd)
      ...
      if(rainhourly == 0){ # putting daily rainfall on midnight (account for local solar time)
        midnight <- which(ZENhr[1:24] == max(ZENhr[1:24]))
        if(midnight <= 24){
    ...
    if(lamb == 1){
      ...
      if(snowmodel == 1){
        return(list(soil = soil, shadsoil = shadsoil, metout = metout, shadmet = shadmet, soilmoist = soilmoist, shadmoist = shadmoist, humid = humid, shadhumid = shadhumid, soilpot = soilpot, shadpot = shadpot, sunsnow = sunsnow, shdsnow = shdsnow, plant = plant, shadplant = shadplant, tcond = tcond, shadtcond = shadtcond, specheat = specheat, shadspecheat = shadspecheat, densit = densit, shaddensit = shaddensit, RAINFALL = RAINFALL, ndays = ndays, elev = ALTT, REFL = REFL[1], longlat = longlat, nyears = nyears, minshade = MINSHADES, maxshade = MAXSHADES, DEP = DEP, SLOPE = SLOPE, ASPECT = ASPECT, HORIZON = HORIZON, dates = tt, dem = dem, dates2 = dates2, PE=PE,BD=BD,DD=DD,BB=BB,KS=KS))
      }else{
        return(list(soil = soil, shadsoil = shadsoil, metout = metout, shadmet = shadmet, soilmoist = soilmoist, shadmoist = shadmoist, humid = humid, shadhumid = shadhumid, soilpot = soilpot, shadpot = shadpot, plant = plant, shadplant = shadplant, tcond = tcond, shadtcond = shadtcond, specheat = specheat, shadspecheat = shadspecheat, densit = densit, shaddensit = shaddensit, RAINFALL = RAINFALL, ndays = ndays, elev = ALTT, REFL = REFL[1], longlat = longlat, nyears = nyears, minshade = MINSHADES, maxshade = MAXSHADES, DEP = DEP, SLOPE = SLOPE, ASPECT = ASPECT, HORIZON = HORIZON, dates = tt, dem = dem, dates2 = dates2, PE=PE,BD=BD,DD=DD,BB=BB,KS=KS))
      }
    }
