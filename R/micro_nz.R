...
#     }
    ...
    for(j in 1:nyears){ # start loop through years
      ...
      if(j==1){
        ...
        VP<-as.numeric(ncvar_get(nc_open(nc),varid="variable",start=start,count))
        nc_close(nc)
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_Rain.nc',sep=""))
        Rain<-as.numeric(ncvar_get(nc_open(nc),varid="variable",start=start,count))
        nc_close(nc)
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_SoilM.nc',sep=""))
        SoilM<-as.numeric(ncvar_get(nc_open(nc),varid="variable",start=start,count))
        nc_close(nc)
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_Wind.nc',sep=""))
        Wind<-as.numeric(ncvar_get(nc_open(nc),varid="variable",start=start,count))
        nc_close(nc)
        ...
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_Tmax.nc',sep=""))
        Tmax<-as.numeric(c(Tmax,ncvar_get(nc_open(nc),varid="variable",start=start,count)))
        nc_close(nc)
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_Tmin.nc',sep=""))
        Tmin<-as.numeric(c(Tmin,ncvar_get(nc_open(nc),varid="variable",start=start,count)))
        nc_close(nc)
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_VP.nc',sep=""))
        VP<-as.numeric(c(VP,ncvar_get(nc_open(bc),varid="variable",start=start,count)))
        nc_close(nc)
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_Rain.nc',sep=""))
        Rain<-as.numeric(c(Rain,ncvar_get(nc_open(nc),varid="variable",start=start,count)))
        nc_close(nc)
        nc<-nc_open(paste(spatial,"/",yearlist[j],'_SoilM.nc',sep=""))
