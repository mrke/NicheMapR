...
#' \item{\code{g_vs_ab_max}{ = 0, \% max leaf vapour conductance, abaxial (bottom of leaf), mol/m2/s, used if leaf = 1}\cr}
#' \item{\code{g_vs_ad_max}{ = 0.3, \% max leaf vapour conductance, adaxial (top of leaf), mol/m2/s}\cr}
#'}
...
#' \item{\code{K_skin}{ = 0, - Hydraulic conductance of skin (kg/(m2 s (J/kg)) - drives liquid water exchange with substrate}\cr}
...
#' \item{\code{K_egg}{ = 2.8e-09, Hydraulic conductance of egg shell (kg/(m2 s (J/kg)) - drives liquid water exchange with substrate}\cr}
...
#' \item{\code{liq_init}{ = Ww_g, Initial mass for egg liquid exchange (g) - total mass of egg which is used as the reference point}\cr}
#' }
...
#' \item{\code{X}{ = 10, Food density (J/cm2) or total food available per hour (J), depending on the 'foodmode' option, single value or hourly vector}\cr}
#' \item{\code{foodmode}{ = 0, 0 assumes X is food density as J/cm2, 1 treats food density as an absolute input to the stomach to simulate single-sized prey items for gape-limited predators}\cr}
#'}
...
#' \item{\code{starvemode}{ = 1, Determines how reproduction buffer is used during starvation, where 0 means it is not used, 1 means it is used before structure is mobilised and 2 means it is used to maximise reserve density}\cr}
#'}
...
#' \item 11 WetFood_g - Wet food intake (g/h)
...
ectotherm <- function(
  ...
  if(errors == 0){
    ...
    if(length(PDIF) != ndays * 24){
      PDIF <- rep(PDIF[1], ndays * 24)
    }
    ...
    if(length(g_vs_ab) != ndays * 24){
      g_vs_ab <- rep(g_vs_ab[1], ndays * 24)
    }
    if(length(g_vs_ad) != ndays * 24){
      g_vs_ad <- rep(g_vs_ad[1], ndays * 24)
    }
    ...
    if(write_input == 1){ # write out input as csv files for debugging
      ...
      write.csv(g_vs_ab, file = "ecto csv input/g_vs_abs.csv")
