...
#' @param FURTHRMK = 0, # user-specified fur thermal conductivity, not used if 0
...
#' @usage solve.endo(AMASS = 1, NGEOM = 4, GMREF = 3, FURTHRMK = 0, ZFURD = 2E-03, ZFURV = 2E-03, TC = 37, TCMAX = 45, TA = 20, TGRD = TA, TSKY = TA, VEL = 0.1, RH = 5, QSOLR = 0, Z = 20, SHADE = 0, NITESHAD = 0,...)
#' @export
#' @details
#' \strong{ Parameters controlling how the model runs:}\cr\cr
#' \code{DIFTOL}{ = 0.001, error tolerance for SIMULSOL (W)}\cr\cr
#'
#' \strong{ Environment:}\cr\cr
#' \code{TAREF}{ = TA, air temperature at reference height (°C)}\cr\cr
#' \code{ELEV}{ = 0, elevation (m)}\cr\cr
#' \code{ABSSB}{ = 0.8, solar absorptivity of substrate (dec \%)}\cr\cr
...
#' \code{PCTDIF}{ = 0.15, proportion of solar radiation that is diffuse (dec \%)}\cr\cr
#'
...
#' \code{UNCURL}{ = 0.1, allows the animal to uncurl to GMULTMAX, the value being the increment GMULT is increased per iteration}\cr\cr
...
#' \code{AK1inc}{ = 0.05, turns on thermal conductivity increase, the value being the increment by which AK1 is increased per iteration (W/mC)}\cr\cr
#' \code{AKMAX}{ = 2.8, maximum flesh conductivity (W/mC)}\cr\cr
#' \code{EXTinc}{ = 0.1, turns on changes in O2 extraction efficiency to simulate panting, the value being the increment by which AK1 is increased per iteration}\cr\cr
#'
...
#' \code{MAXPTVEN}{ = 0, maxium fraction of surface area that is ventral (dec \%)}\cr\cr
...
#' \code{REFLD}{ = 0.2, fur reflectivity dorsal (dec \%)}\cr\cr
#' \code{REFLV}{ = 0.2, fur reflectivity ventral (dec \%)}\cr\cr
#'
...
#' \code{AK1}{ = 0.9, # initial thermal conductivity of flesh (0.412 - 2.8 W/mC)}\cr\cr
#' \code{AK2}{ = 0.230, # conductivity of fat (W/mC)}\cr\cr
...
#' \code{PCTBAREVAP}{ = 2.8, maximum flesh conductivity (W/mC)}\cr\cr
...
#' \code{RQ}{ = 0.80, # respiratory quotient (dec \%)}\cr\cr
...
solve.endo <- function(
  ...
  ELEV = 0, # elevation (m)
  ABSSB = 0.8, # solar absorptivity of substrate (dec %)
  ...
  PCTDIF = 0.15, # proportion of solar radiation that is diffuse (dec %)
  ...
  RAISETC = 0.1, # turns on core temperature elevation, the value being the increment by which SKINW is increased per iteration
  ...
  AK1inc = 0.05, # turns on thermal conductivity increase, the value being the increment by which AK1 is increased per iteration
  AKMAX = 2.8, # maximum flesh conductivity
  EXTinc = 0.1, # turns on changes in O2 extraction efficiency to simulate panting, the value being the increment by which AK1 is increased per iteration
  ...
  MAXPTVEN = 0, # maxium fraction of surface area that is ventral (dec %)
  AWING = 0, # area of wing, to do
  PTCOND = 0, # % of body area touching the substrate

  # fur properties
  FURTHRMK = 0, # user-specified fur thermal conductivity, not used if 0
  ...
  REFLD = 0.2,  # fur reflectivity dorsal (dec %)
  REFLV = 0.2,  # fur reflectivity ventral (dec %)
  ...
  AK2 = 0.230, # conductivity of fat (W/mC)
  ...
  while(QGEN < QBASAL){
    ...
    # output
    E4B1 <- IRPROP.out[1] # exponential integral, E4, of optical thickness, B1 (m), i.e. E4(B1), not sure this is used anymore
    KEFARA <- IRPROP.out[2:4] # effective thermal conductivity of fur array, mean, dorsal, ventral(W C-1 m-1)
    ...
    REFLFR <- IRPROP.out[23:25] # fur reflectivity array, mean, dorsal, ventral (dec %)
    ...
    ABSAND <- 1 - REFLD # solar absorptivity of dorsal fur (dec %)
    ABSANV <- 1 - REFLV # solar absorptivity of ventral fur (dec %)
    ...
    for(S in 1:2){
      ...
      SKYIN <- SKYRAD #(C3 + C6) * (TA + 273.15) ^ 4
      ...
      # Nat comment - general coding issue (Jan 13): Not sure if should do conduction to surface or below surface temperature? Note that substrate conductivity is set in code.
      # Nat comment - for conduction to tree trunk, only have tree surface temperature - set this here.
    ...
    # Now compute a "mixed" for all the parts/components = (dorsal value *(FASKY+FAVEG+FATOBJ))+(ventral value*FAGRD)
  ...
  SWEAT.G.H <- (SIMULSOL.out[12] + SIMULSOL.out[12]) * 0.5 / HTOVPR * 1000 * 3600
  EVAP.G.H <- ZBRENT.out[3] * 3600 + SWEAT.G.H
  endo.out <- as.matrix(cbind(TC, SIMULSOL.out[1,1], SIMULSOL.out[2,1], SIMULSOL.out[1,2], SIMULSOL.out[2,2], SIMULSOL.out[1,3], SIMULSOL.out[2,3], SIMULSOL.out[1,4], SIMULSOL.out[2,4], SIMULSOL.out[1,5], SIMULSOL.out[2,5], SIMULSOL.out[1,6], SIMULSOL.out[2,6], SIMULSOL.out[1,7], SIMULSOL.out[2,7], SIMULSOL.out[1,8], SIMULSOL.out[2,8], SIMULSOL.out[1,9], SIMULSOL.out[2,9], SIMULSOL.out[1,10], SIMULSOL.out[2,10], SIMULSOL.out[1,11], SIMULSOL.out[2,11], SIMULSOL.out[1,12], SIMULSOL.out[2,12], SIMULSOL.out[1,13], SIMULSOL.out[2,13], SIMULSOL.out[1,14], SIMULSOL.out[2,14], ZBRENT.out, GMULT, SKINW, SWEAT.G.H, EVAP.G.H, EXTREF, AK1, TA, TGRD, TCONDSB, TSKY, VEL, RH, QSOLR))
  colnames(endo.out) <- c("TC", "TFA_D", "TFA_V", "TSKIN_D", "TSKIN_V", "QCONV_D", "QCONV_V", "QCOND_D", "QCOND_V", "QGENNET_D", "QGENNET_V", "QSEVAP_D", "QSEVAP_V", "QRAD_D", "QRAD_V", "QSLR_D", "QSLR_V", "QRSKY_D", "QRSKY_V", "QRBSH_D", "QRBSH_V", "QRVEG_D", "QRVEG_V", "QRGRD_D", "QRGRD_V", "NTRY_D", "NTRY_V", "SUCCESS_D", "SUCCESS_V", "RESPFN","QRESP","GEVAP", "PCTO2", "PCTN2", "PCTCO2", "RESPGEN", "O2STP", "O2MOL1", "N2MOL1", "AIRML1", "O2MOL2", "N2MOL2", "AIRML2", "AIRVOL", "GMULT", "SKINW", "SWEAT.G.H", "EVAP.G.H", "EXTREF", "AK", "TA", "TGRD", "TCONDSB", "TSKY", "VEL", "RH", "QSOLR")
