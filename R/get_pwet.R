#' Function to find effective \% of surface area that is wet from experimental water loss data
#'
#' A set of equations for computing the percentage of the total surface area of an organism that
...
#' @param mass.g = 26.7, Mass of animal, g
...
#' @param T_b = 25, Core body temperature, deg C
#' @param T_s = 24.99, Skin (surface) temperature, deg C
#' @param T_a = 24.98, Air temperature, deg C
...
#' @param T_ref_O2 = 25, Reference temperature for oxygen consumption rate correction (deg C)
#' @param V_air_STP = V_O2_STP / (0.2094 * E_O2 / 100), Ventilation rate at STP, L/s
#' @param T_ref_vent = 25, Reference temperature for ventilation rate correction (deg C)
...
#' @return A_wet
#' @return pct_wet
#' @usage get_pwet(cut.known = 1, E_tot = 0.344 / 60 / 1000, E_cut = 0.108 / 60 / 1000, T_b = 25, RHin = 30, vel = 0.0021)
#' @examples
#' library(NicheMapR)
#'
#'
#' @export
get_pwet <- function(cut.known = 0, resp.known = 0, eye.known = 0, vent.known = 0,
  press.known = 0, A_tot.known = 0,  E_tot = 0.344 / 60 / 1000,
  E_cut = 0.108 / 60 / 1000, E_resp = 0.0371 / 60 / 1000,
  E_eye = 0.199 / 60 / 1000, mass.g = 26.7, A_tot = 0.00894,
  fcond = 0.25, feye = 0.002, eye_frac = 0.58, T_b = 25, T_s = 25 - 0.001,
  T_a = 25 - 0.002, vel = 0.0021, RHex = 100, RHin = 30, E_O2 = 15,
  RQ = 0.766,  M_1 = 0.013, M_2 = 0.8, M_3 = 0.0038,
  V_O2_STP = M_1 * mass.g ^ M_2 * 10 ^ (M_3 * T_b) / 3600 / 1000,
  T_ref_O2 = 30, V_air_STP = V_O2_STP / (0.2094 * E_O2 / 100), rho_flesh = 1000,
...
# bp	= 101325#Barometric pressure (pascal)
  ...
  delta_T <- T_s - T_a # temperature difference between skin and air, deg C
  if(abs(delta_T) < 0.01){delta_T <- 0.01 * sign(delta_T)} # impose lower limit of 0.01 deg C
  ...
  if(HC_free < 5){HC_free <- 5} # don't let it go below 5, as this is unrealistic
  ...
  # forced convection
  PR_forc <-	0.72 # Prandlt number for forced convection, - (FROM ANCORR)
  SC_forc <-	0.6 # Schmidt number for forced convection, - (FROM ANCORR)
  NU_forc <- 0.35 * RE ^ 0.6	# Nusslet number for forced convection, - (FROM ANCORR)
  ...
  if(eye.known  == 0){
    ...
    E_cut <- E_cut - E_eye # take any ocular water loss from the estimate of cutaneous
  }

  # compute actual skin resistance (unlike pct_wet, this does not include eyes)
  ...
  # compute % of skin area that is acting like a free-water surface, p_wet, and associated areas
  A_eff <- (E_cut / 1000) / (HD * (rho_H2O_skin - rho_H2O_air)) # effective area for evaporation, m2
  pct_wet = A_eff / (A_tot - A_tot * fcond - A_eye * eye_frac + A_eye * (1 - eye_frac)) * 100
  A_wet = pct_wet / 100 * A_tot

  result <- cbind(E_tot, E_cut, E_resp, E_eye, R_a, R_s, A_tot, A_eye, A_eff, A_wet, pct_wet)
  colnames(result) <- c('E_tot', 'E_cut', 'E_resp', 'E_eye', 'R_a', 'R_s', 'A_tot', 'A_eye', 'A_eff', 'A_wet', 'pwet')
  return(result)
}
