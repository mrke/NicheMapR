...
Refhyt <- 200 # Reference height (cm), reference height at which air temperature, wind speed and relative humidity input data are measured
...
microinput<-c(julnum,RUF,ERR,Usrhyt,Numtyps,Numint,Z01,Z02,ZH1,ZH2,idayst,ida,HEMIS,ALAT,AMINUT,ALONG,ALMINT,ALREF,slope,azmuth,ALTT,CMH2O,microdaily,tannul,EC,VIEWF,snowtemp,snowdens,snowmelt,undercatch,rainmult,runshade,runmoist,maxpool,evenrain,snowmodel,rainmelt,writecsv,densfun,Refhyt)
...
# # 9 SNOWMELT - snow melt (ml/h)
# # 10 POOLDEP - depth of water pooling on surface (mm)
# # 11 PCTWET - fraction of surface that is wet (%)
# # 12 ZEN - zenith angle of sun (degrees - 90 = below the horizon)
# # 13 SOLR - solar radiation (W/m2)
# # 14 TSKYC - sky radiant temperature (deg C)
# # 15 DEW - presence of dew on ground surface
# # 16 FROST - presence of frost on ground surface
...
# # 1 JULDAY - day of year
# # 2 TIME - time of day (mins)
# # 3-12 D0cm ... - soil temperature (deg C) at each of the 10 specified depths
# # soilmoist and shadmoist variables:
# # 1 JULDAY - day of year
# # 2 TIME - time of day (mins)
# # 3-12 WC0cm ... - soil moisture (decimal %) at each of the 10 specified depths
# # humid and shadhumid variables:
# # 1 JULDAY - day of year
# # 2 TIME - time of day (mins)
# # 3-12 RH0cm ... - soil humidity (decimal %) at each of the 10 specified depths
# # soilpot and shadpot variables:
# # 1 JULDAY - day of year
# # 2 TIME - time of day (mins)
# # 3-12 PT0cm ... - soil water potential (J/kg = bar/100 = kPa = cm*10) at each of the 10 specified depths
# ###################plots#################################################
# library(lattice) # library to do lattice plots
# # demonstrating git
# # plotting above-ground conditions in minimum shade
# with(metout,{xyplot(TALOC + TAREF ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Air Temperature (deg C)", auto.key=list(columns = 2), as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(RHLOC + RH ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Relative Humidity (%)", auto.key=list(columns = 2), as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(VLOC ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Wind Speed (m/s)", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(ZEN ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Zenith Angle of Sun (deg)", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(SOLR ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Solar Radiation (W/m2)", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(TSKYC ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Sky Temperature (deg C)", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(SNOWMELT ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "snowmelt, mm", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(POOLDEP ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "water pooling on surface, mm", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(PCTWET ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "soil surface wetness, %", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(DEW + FROST ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "frost and dew presence", as.table = TRUE, type = "h", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(SNOWFALL ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "snowfall, mm", as.table = TRUE, type = "h", main=paste(minshade,"% shade",sep=""))})
# with(metout,{xyplot(SNOWDEP ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "snow depth, cm", as.table = TRUE, type = "l", main=paste(minshade,"% shade",sep=""))})
#
# # plotting above-ground conditions in maximum shade
# shadmet<-as.data.frame(shadmet)
# with(shadmet,{xyplot(TALOC + TAREF ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Air Temperature (deg C)", auto.key=list(columns = 2), as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(RHLOC + RH ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Relative Humidity (%)", auto.key=list(columns = 2), as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(VLOC ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Wind Speed (m/s)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(ZEN ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Zenith Angle of Sun (deg)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(SOLR ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Solar Radiation (W/m2)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(TSKYC ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Sky Temperature (deg C)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(SNOWMELT ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "snowmelt (litres)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(POOLDEP ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "water pooling on surface (mm)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(PCTWET ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "soil surface wetness (%)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(DEW + FROST ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "frost and dew presence", auto.key=list(columns = 2), as.table = TRUE, type = "h", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(SNOWFALL ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "snowfall (mm)", as.table = TRUE, type = "h", main=paste(maxshade,"% shade",sep=""))})
# with(shadmet,{xyplot(SNOWDEP ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "snow depth, (cm)", as.table = TRUE, type = "l", main=paste(maxshade,"% shade",sep=""))})
#
# # plotting soil for minimum shade
# soil<-as.data.frame(soil)
# with(soil,{xyplot(D0cm + D2.5cm + D5cm + D10cm + D15cm + D20cm + D30cm + D50cm + D100cm + D200cm ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Soil Temperature (deg C)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(minshade,"% shade"))})
#
# # plotting soil for maximum shade
# shadsoil<-as.data.frame(shadsoil)
# with(shadsoil,{xyplot(D0cm + D2.5cm + D5cm + D10cm + D15cm + D20cm + D30cm + D50cm + D100cm + D200cm ~ TIME | as.factor(JULDAY),xlab = "Time of Day (min)", ylab = "Soil Temperature (deg C)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(maxshade,"% shade"))})
#
# # plotting soil moisture for minimum shade
# soilmoist<-as.data.frame(soilmoist)
# with(soilmoist,{xyplot(WC0cm*100 + WC2.5cm*100 + WC5cm*100 + WC10cm*100 + WC15cm*100 + WC20cm*100 + WC30cm*100 + WC50cm*100 + WC100cm*100 + WC200cm*100 ~ soil$TIME | as.factor(soil$JULDAY),xlab = "Time of Day (min)", ylab = "Soil Moisture (%)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(minshade,"% shade"))})
#
# # plotting soil moisture for maximum shade
# shadmoist<-as.data.frame(shadmoist)
# with(shadmoist,{xyplot(WC0cm*100 + WC2.5cm*100 + WC5cm*100 + WC10cm*100 + WC15cm*100 + WC20cm*100 + WC30cm*100 + WC50cm*100 + WC100cm*100 + WC200cm*100 ~ soil$TIME | as.factor(soil$JULDAY),xlab = "Time of Day (min)", ylab = "Soil Moisture (%)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(maxshade,"% shade"))})
#
# # plotting soil humidity for minimum shade
# humid<-as.data.frame(humid)
# with(humid,{xyplot(RH0cm*100 + RH2.5cm*100 + RH5cm*100 + RH10cm*100 + RH15cm*100 + RH20cm*100 + RH30cm*100 + RH50cm*100 + RH100cm*100 + RH200cm*100 ~ soil$TIME | as.factor(soil$JULDAY),xlab = "Time of Day (min)", ylab = "Soil Humidity (%)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(minshade,"% shade"))})
#
# # plotting soil humidity for maximum shade
# shadhumid<-as.data.frame(shadhumid)
# with(shadhumid,{xyplot(RH0cm*100 + RH2.5cm*100 + RH5cm*100 + RH10cm*100 + RH15cm*100 + RH20cm*100 + RH30cm*100 + RH50cm*100 + RH100cm*100 + RH200cm*100 ~ soil$TIME | as.factor(soil$JULDAY),xlab = "Time of Day (min)", ylab = "Soil Humidity (%)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(maxshade,"% shade"))})
#
# # plotting soil water potential for minimum shade
# soilpot<-as.data.frame(soilpot)
# with(soilpot,{xyplot(PT0cm/100 + PT2.5cm/100 + PT5cm/100 + PT10cm/100 + PT15cm/100 + PT20cm/100 + PT30cm/100 + PT50cm/100 + PT100cm/100 + PT200cm/100 ~ soil$TIME | as.factor(soil$JULDAY),xlab = "Time of Day (min)", ylab = "Soil Water Potential (bar = kPa = J/kg*100 = cm/10)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(minshade,"% shade"))})
#
# # plotting soil water potential for maximum shade
# shadpot<-as.data.frame(shadpot)
# with(shadpot,{xyplot(PT0cm/100 + PT2.5cm/100 + PT5cm/100 + PT10cm/100 + PT15cm/100 + PT20cm/100 + PT30cm/100 + PT50cm/100 + PT100cm/100 + PT200cm/100 ~ soil$TIME | as.factor(soil$JULDAY),xlab = "Time of Day (min)", ylab = "Soil Water Potential (bar = kPa = J/kg*100 = cm/10)", auto.key=list(columns = 5), as.table = TRUE, type = "l", main=paste(maxshade,"% shade"))})
#
#
#
#
