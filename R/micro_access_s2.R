...
micro_access_s2 <- function(
  ...
  # --- 1. Input validation ---
  errors <- 0
  reference_height <- 2 # Reference height (m): ACCESS-S2 data is at 2 m screen height

  # access_s2-specific checks
  ...
  errors <- errors + validate_micro_inputs(depths, surface_reflectivity, slope, aspect, horizon_angles,
    ...
    maxima_times, minima_times, minimum_shade, maximum_shade, mineral_conductivity, mineral_density, mineral_heat_capacity, bulk_density,
  ...
  if(errors==0){ # continue
    ...
    if(timezone==1){ # this now requires registration
      ...
      solar_noon_longitude<-(geonames::GNtimezone(longlat[2],longlat[1])[4])*-15
    ...
    if(save != 2){
      ...
      reference_humidity_min <- (vapr * 100 / es) * 100
      ...
      reference_humidity_max <- (vapr * 100 / es) * 100
    ...
    if(is.na(ALTITUDES)!=TRUE){
      ...
      e <- WETAIR(db = Tmax, rh = reference_humidity_min)$e
      reference_humidity_min <- (e / es) * 100
      reference_humidity_min[reference_humidity_min>100]<-100
      reference_humidity_min[reference_humidity_min<0]<-0.01
      es <- WETAIR(db = reference_temperature_min, rh = 100)$esat
      e <- WETAIR(db = Tmin, rh = reference_humidity_max)$e
      ...
      dates <- seq.POSIXt(as.POSIXct(paste0(dstart, "00:00"), format = "%d/%m/%Y %H:%M", tz = tzone), as.POSIXct(paste(dfinish, "23:00"), format = "%d/%m/%Y", tz = tzone)+23*3600*2, by = 'hours')[1:(length(reference_temperature_max) * 24)] # careful about daylight savings!
      dates2 <- round(as.POSIXct(seq(as.Date(dstart, format = "%d/%m/%Y", tz = tzone), as.Date(dfinish, format = "%d/%m/%Y", tz = tzone) + 24*3600, by = 'days')[1:(length(reference_temperature_max))], tz = tzone), "days")
      return(build_micro_return(out, rainfall, ndays, elevation, surface_reflectivity,
        ...
        depths, dates, dates2, air_entry_water_potential, soil_bulk_density, soil_mineral_density, campbell_b_parameter, saturated_hydraulic_conductivity, dem = dem, diffuse_frac = diffuse_frac,
