...
micro_access_s2 <- function(
  ...
  # --- 1. Input validation ---
  errors <- 0
  Refhyt <- 2 # Reference height (m): ACCESS-S2 data is at 2 m screen height
  ...
  errors <- errors + validate_micro_inputs(DEP, REFL, slope, aspect, hori,
    SLE, ERR, RUF, D0, Usrhyt, Refhyt, EC, CMH2O,
    TIMAXS, TIMINS, minshade, maxshade, Thcond, Density, SpecHeat, BulkDensity)
  ...
  if(errors==0){ # continue
    ...
    if(is.na(ALTITUDES)!=TRUE){
      ...
      microinput <- build_microinput(ndays, RUF, ERR, Usrhyt, Refhyt, Numtyps,
        ...
        spinup, dewrain = 0, moiststep = 360, maxsurf, ndmax)
      ...
      dates <- seq.POSIXt(as.POSIXct(paste0(dstart, "00:00"), format = "%d/%m/%Y %H:%M", tz = tzone), as.POSIXct(paste(dfinish, "23:00"), format = "%d/%m/%Y", tz = tzone)+23*3600*2, by = 'hours')[1:(length(TMAXX) * 24)] # careful about daylight savings!
      dates2 <- round(as.POSIXct(seq(as.Date(dstart, format = "%d/%m/%Y", tz = tzone), as.Date(dfinish, format = "%d/%m/%Y", tz = tzone) + 24*3600, by = 'days')[1:(length(TMAXX))], tz = tzone), "days")
      return(build_micro_return(out, RAINFALL, ndays, ALTT, REFL,
        longlat = c(x[1], x[2]), nyears, timeinterval = ndays, MINSHADES, MAXSHADES,
        DEP, dates, dates2, PE, BD, DD, BB, KS, dem = dem, diffuse_frac = diffuse_frac,
        snowmodel, lamb))
    }else{
