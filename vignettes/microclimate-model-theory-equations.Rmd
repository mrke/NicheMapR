...
The factor $\left( {\frac{a}{r}} \right) ^2$ is approximated in the model as $1+2\epsilon \cos (\omega d)$ where $\omega=\frac{2 \pi} {365}$,  \omega is the eccentricity of the earth’s orbit (default value of 0.01675) and *d* is the day of the year (1-365). From the astronomical triangle, the term $\cos Z = \cos \phi \cos \delta h + \sin \phi \sin \delta$, where \phi is the latitude, \delta is the solar declination and *h* is the solar hour angle. The solar declination $\delta = \arcsin (0.39784993 \sin \zeta)$ where the ecliptic longitude of the earth in its orbit $\zeta = \omega (*d* - 80 ) + 2 \epsilon [\sin (\omega d) - \sin (\omega 80) ]$. The hour angle $h = 15(t_d – t_{sn})$ degrees, with $t_d$ the local standard time of day and tsn the local standard time of true solar noon. The value of $t_{sn}$ is calculated by adding/subtracting 4 min for each degree of longitude west/east of the reference longitude for the local time zone. The hour angle at sunset $H_+=\arccos [-\tan \delta \tan \phi ]$ while the hour angle at sunrise $H_-=-H_+$. In the polar zones, where the (absolute) latitude \phi>66°30', there may be no sunrise/sunset but rather a long day ($H_+$ = \pi) or a long night ($H_+$ = 0).
...
### 1.2.1 Direct Component of Terrestrial Radiation
...
In the model, no generic altitudinal correction is made for the effect of precipitable water vapour as its vertical distribution is highly variable, and AW(h) is assumed by default to equal 1 cm for all elevations but can be changed by the user. Using the Gates and Harrop (1963) coefficients, the total monochromatic extinction coefficient for water vapour absorption $$_\lambda \tau_W (Z_a,h)=_\lambda \tau_W  \sqrt{ m(Z_a) w A_W(h) } $$
...
### 1.2.2 Scattered Component of Terrestrial Radiation
...
where $A(\lambda)$ is the albedo of the underlying surface and the functions $\gamma_l$, $\gamma_r$ and $\bar{s}$ ̅ are calculated using the Dave-Warten subroutine (Dave and Warten 1968). In the ultraviolet region, <=360 nanometers, 
...
## 1.3 Slope, aspect and hill-shade effects
...
### 1.2.3 Total solar radiation reaching a surface
...
For longwave radiation from clear skies, ARAD, the sky emissivity is approximated as $\epsilon_{SKY} = 1.72 \left( \frac{e_A}{T_A} \right)^{\frac{1}{7}}$ (Campbell Gaylson S. and Norman 1998, eqn 10.10), where $T_A$ is the shaded air temperature (at reference height, 1.2 m) in Kelvin and $e_A$ is the vapour pressure of the air in kilopascals (see section 5). Thus, given a percentage cloud clover $CLD$, 
...
# 3. Hourly interpolation of weather data
...
# 4. Vertical air temperature and wind speed profiles
...
# 5. Hydric and thermal properties of air, and surface evaporation
...
# 6. Soil heat balance and soil thermal properties
