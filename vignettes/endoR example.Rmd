...
output:
  pdf_document:
    toc: true
vignette: >
  %\VignetteIndexEntry{endoR example}
...
This document provides an overview of an R implementation of the NicheMapR endotherm model. This implementation modularises the original version such that many of the internal subroutines and functions can be called separately. The idea is that the user has more flexibility in setting the model up for the behavioural and physiological responses of a particular animal.
...
This subroutine computes the feasible hair spacing and the three parameters needed for computing conduction and infrared radition through the fur.

```{r}
# environmental input
TA <- 20 # air temperature, for calculation of conductivity of air (degrees C)
...
MAXPTVEN <- 0.3 # maxium fraction of surface area that is ventral (dec %)
...
REFLD <- 0.301  # fur reflectivity dorsal (dec %) 
REFLV <- 0.301  # fur reflectivity ventral (dec %)
...
plot(KEFARAs ~ DHAIRs, type = 'p', pch = 16, ylab = 'effective fur conductivity, W C-1 m-1', xlab = 'hair diameter, um')
```
...
plot(KEFARAs ~ RHOs, type = 'p', pch = 16, ylab = 'effective fur conductivity, W C-1 m-1', xlab = 'hair density, 1/cm2')
```

Example analysis of fur density on effective thermal conductivity of fur.

```{r}
ZFURs <- seq(0, 50, 1) # hair densities (1/cm2)
KEFARAs <- NULL
...
plot(KEFARAs ~ ZFURs, type = 'p', pch = 16, ylab = 'effective fur conductivity, W C-1 m-1', xlab = 'fur depth, mm')
```
...
PTCOND <- 0.1 # fraction of body in contact with substrate (dec %)
...
SHADE <- SHADE # shade (dec %) (defined above for F_FACTOR)
...
ABSAND <- 0.8 # solar absorptivity of dorsal fur (dec %)
ABSANV <- 0.8 # solar absorptivity of ventral fur (dec %)
...
```{r}
# input
TS <- 33 # skin temperature (deg C)
TENV <- 20 # air temperature (deg C)
TFA <- 10 # fur/air interface temperature (deg C)
...
TA <- 20 # air temperature (deg C)
RELHUM <- 20 # relative humidity (%)
TC <- 37 # core temperature (deg C)
TSKIN <- 33 # skin temperature (deg C)
...
Setup and the SIMULSOL algorithm to simultaneously solve for skin temperature $T_skin$ and the temperature of the fur/feather interface $T_fa$ given an environment, posture and skin wetness.

```{r}
# environment
TC <- 37 # core temperature (deg C)
TAREF <- TA # 1.2 m reference air temperature (deg C)
TGRD <- TA # ground temperature (deg C)
TSKY <- TA # sky temperature (deg C)
TCONDSB <- TA # surface temperature for conduction (deg C)
TBUSH <- TA # bush temperature (deg C)
...
TVEG <- TAREF # assume vegetation casting shade is at 1.2 m (reference) air temperature (deg C)
...
Finally, find a value of QRESP to balance the heat budget by solving RESPFUN with ZBRENT.

```{r}
# define basal metabolic rate
QBASAL <- (70 * AMASS ^ 0.75) * (4.185 / (24 * 3.6)) # heat generation (W)
DELTAR <- 0 # offset between air temeprature and breath (deg C)
