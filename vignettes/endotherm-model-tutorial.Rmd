---
title: "Endotherm Model Tutorial III: example simulations"
author: "Michael Kearney"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Endotherm Model Example}
...
# above 40 °C = 30% relative_humidity_reference at 40 °C
...
micromet_lowshade <- as.data.frame(micro$micromet_lowshade) # unshaded above-ground conditions
soil_temperature_lowshade <- as.data.frame(micro$soil_temperature_lowshade) # unshaded below-ground soil temperatures
micromet_highshade <- as.data.frame(micro$micromet_highshade) # shaded above-ground conditions
soil_temperature_highshade <- as.data.frame(micro$soil_temperature_highshade) # shaded below-ground soil temperatures
dates <- micro$dates

TAs <- micromet_lowshade$air_temperature_local # air temperatures at height of animal (°C)
TAREFs <- micromet_lowshade$air_temperature_reference # air temperatures at reference height (°C)
TSKYs <- micromet_lowshade$sky_temperature # sky temperatures (°C)
TGRDs <- soil_temperature_lowshade[["depth_0cm"]] # surface temperatures (°C)
VELs <- micromet_lowshade$wind_speed_local # wind speeds at animal height (m/s)
RHs <- micromet_lowshade$relative_humidity_local # relative humidity at animal height (%)
QSOLRs <- micromet_lowshade$solar_radiation # solar radiation (W/m2)
Zs <- micromet_lowshade$zenith_angle # zenith angle of the sun (degrees)
