...
This subroutine computes the feasible hair spacing and the three parameters needed for computing conduction and infrared radiation through the fur.
...
Setup and the SIMULSOL algorithm to SIMULtaneously SOLve for skin temperature $T_skin$ and the temperature of the fur/feather interface $T_fa$ given an environment, posture and skin wetness. SIMULSOL is the core algorithm for computing the heat balance.
...
```{r}
# # NEST properties: if nest THICKNESS = 0.0, no nest is assumed
# NUMANIMLS=1 # Shelter/Nest # animals in nest to adjust # present for xtra heat production. If <1, no nest calc. 
# NESLOC=0 #  If nest/rest place above ground = 0, If nest/rest place below ground = 1,If nest=1,set 'Burrow OK'='N',so stay @ 1 depth
# BURDEP=2 # If nest is below ground,'1',then how deep is the nest, i.e., what is the node number (2-10)? Example: node 6 is at 20 cm. (see bottom line below) #!
# NESTYP=6 # Shelter/nest type: 0=FLAT,1=CUPP,2=CYLN,3=HFCL,4=SPHR,5=DOME,6=NONE. CYLN=HOLLOW FULL CYL; HFCL=HOLLOW HALF CYLINDER; SPHR=HOLLOW SPHERE ) 
# NESUSE=0 # NEST/SHELTER USE WHEN INACTIVE (NOT HIBERNATING)?  no=0, yes=1
# # Shelter/Nest properties  (Hominid Paleoshelter) Used when nest type is not 'NONE' and multiplier >= 1   
# SHELEN=0.0 # Length(m) #!
# REFSHEL=0.29 # Solar reflectivity(decimal: 1.0 = 100%)
# SPHTNS=2000 # Specific Heat (J/kg-C)
# HYTNES=-10 # Nest height rel. to ground surf.(m) + = below surface, - = above surface 
#NESTYP <- 1 # for nest calculations - 0 = none, 1 = flat, 2 = cup, 3 = cylinder, 4 = half cylinder, 5 = sphere, 6 = dome

SIG <- 5.6697E-8

# DEFINE THE SHELTER GEOMETRY FOR CONVECTIVE HEAT TRANSFER AND FOR SURF. & SILHOUETTE AREAS CALCULATIONS
if(NESTYP == 1){
 # FLAT SHELTER/NEST
 NGSHEL = 0
 # SHELTER OUTER SURFACE AREA - COMPUTE AREA=pi *D*L
 SHELAREA <- RONEST * SHELEN
 # SILHOUETTE AREA = f(AZIMUTH ANGLE) ?=TO BE DONE. CURRENTLY ONLY NORMAL TO SUN
 SHELSIL <- RONEST * SHELEN * cos(ZEN)
 # IR UP & DOWN
 C3SHEL <- SHELAREA*0.5*EMISHEL*SIG
 C4SHEL <- C3SHEL
 # NEST MASS TO GET AN ESTIMATE OF TOLERANCE FOR STEADY STATE SOLUTION
 SHELVOL <- 2 * RONEST * SHELEN * TKNEST
 NESMAS <- DENEST * SHELVOL
 AV <- 0
}

if(NESTYP == 2){
 # CUP-SHAPED SHELTER/NEST
 NGSHEL <- 2
 # SHELTER OUTER SURFACE AREA - COMPUTE AREA=(4.0*pi *R**2/2.0
 SHELAREA <- 2 * pi * RONEST ^ 2
 # SILHOUETTE AREA = f(ZENITH, AZIMUTH ANGLE) ?=TO BE DONE. CURRENTLY ONLY NORMAL TO SUN
 SHELSIL <- 0.5 * pi * RONEST ^ 2 * (1 + cos(ZEN))
 # IR UP & DOWN
 C3SHEL <- (pi * RONEST ^ 2) * EMISHEL * SIG
 C4SHEL <- SHELAREA * EMISHEL * SIG
 # NEST MASS TO GET AN ESTIMATE OF TOLERANCE FOR STEADY STATE SOLUTION
 SHELVOL <- ((4 / 3) * pi * RONEST ^ 3 - (4 / 3) * pi * RINEST ^ 3) / 2
 NESMAS <- DENEST * SHELVOL
 AV <- SHELAREA * 0.2
}

if(NESTYP == 3){
 # CYLINDRICAL SHELTER/NEST
 NGSHEL <- 1
 # SHELTER OUTER SURFACE AREA - COMPUTE AREA=pi *D*L
 SHELAREA <- pi * 2 * RONEST * SHELEN
 # SILHOUETTE AREA = f(AZIMUTH ANGLE) ?=TO BE DONE. CURRENTLY ONLY NORMAL TO SUN
 SHELSIL <- 2 * RONEST * SHELEN
 # IR UP & DOWN
 C3SHEL <- SHELAREA * 0.5 * EMISHEL * SIG
 C4SHEL <- C3SHEL
 # NEST MASS TO GET AN ESTIMATE OF TOLERANCE FOR STEADY STATE SOLUTION
 SHELVOL <- pi * RONEST ^ 2 * SHELEN - pi * RINEST ^ 2 * SHELEN
 NESMAS <- DENEST * SHELVOL
 AV <- 0
}

if(NESTYP == 4){
 # HALF CYLINDRICAL SHELTER/NEST: CYLINDRICAL 'DOME'
 NGSHEL <- 1
 # SHELTER OUTER SURFACE AREA - COMPUTE AREA=pi *D*L/2 + pi *R**2 (2 HALF ENDS)
 SHELAREA = (pi * 2 * RONEST * SHELEN) / 2 + (2 * pi * RONEST ^ 2) / 2
 # SILHOUETTE AREA = f(ZENITH) ONLY FOR NOW  ASSUMING LONG AXIS NORMAL TO SUN'S RAYS
 SHELSIL <- RONEST * SHELEN * (1 + cos(ZEN))
 # IR UP & DOWN
 C3SHEL <- SHELAREA * 0.5 * EMISHEL * SIG
 C4SHEL <- C3SHEL
 # NEST MASS TO GET AN ESTIMATE OF TOLERANCE FOR STEADY STATE SOLUTION
 SHELVOL <- (pi * RONEST ^ 2 * SHELEN - pi * RINEST ^ 2 * SHELEN) / 2
 NESMAS <- DENEST * SHELVOL
 AV <- 0
}

if(NESTYP == 5){ 
 # SPHERICAL SHELTER/NEST
 NGSHEL <- 2
 # SHELTER OUTER SURFACE AREA - COMPUTE AREA=4.0*pi *R**2
 SHELAREA = 4 * pi * RONEST ^ 2
 # SILHOUETTE AREA
 SHELSIL = pi * RONEST ^ 2
 # IR UP & DOWN
 C3SHEL <- SHELAREA * 0.5 * EMISHEL * SIG
 C4SHEL <- C3SHEL
 # NEST MASS TO GET AN ESTIMATE OF TOLERANCE FOR STEADY STATE SOLUTION
 SHELVOL = (4 / 3) * pi * RONEST ^ 3 - (4 / 3) * pi * RINEST ^ 3
 NESMAS <- DENEST * SHELVOL
 AV <- 0
}

if(NESTYP == 6){
 # DOME-SHAPED SHELTER/NEST (I.E. CUP ABOVE ANIMAL
 NGSHEL <- 2
 # SHELTER OUTER SURFACE AREA -> COMPUTE AREA=(2.0*pi *R**2/2.0
 SHELAREA <- 2.0*pi *RONEST^ 2.0
 # SILHOUETTE AREA = f(ZENITH ANGLE)
 SHELSIL <- real(0.5*pi *RONEST**2,4) * (1. + COS(ZEN))
 # IR UP & DOWN
 C3SHEL <- SHELAREA*EMISHEL*SIG
 C4SHEL <- (pi *RONEST^ 2.0)*EMISHEL*SIG
 # NEST MASS TO GET AN ESTIMATE OF TOLERANCE FOR STEADY STATE SOLUTION
 SHELVOL <- ((4 / 3) * pi * RONEST ^ 3 - (4 / 3) * pi * RINEST ^ 3) / 2
 NESMAS <- DENEST * SHELVOL
 AV <- 0
...
# INITIAL GUESSES FOR OUTER TEMPERATURE OF SHELTER
X1 <- TSKY - 10
X2 <- TA + 10
TOL1 <- NESMAS * 0.01
...
#QSOLAR <- 0
if(QSOLAR > 0){
 # SENDING DORSAL ABSORPTIVITY, VENTRAL ABSORPTIVITY,SILHOUETTE AREA,TOTAL AREA
 SHELDABS = ABSHEL
 SHELVABS = ABSHEL
 SOLAR.out <- SOLAR(SHELAREA, SHELDABS, SHELVABS, ABSSB, SHELSIL, PCTDIF, QNORM, SHADE, AWING, QSOLR, FASKY, FATOBJ, FAVEG)
 QDORSL <- SOLAR.out[7] # dorsal direct solar radiation (W)
 QVENTR <- SOLAR.out[8] # ventral diffuse solar radiaton (W)
}else{
 QDORSL <- 0
...
SHELSOLR = QDORSL + QVENTR

# GET THE IR FLUXES FROM SKY AND GROUND OUTSIDE THE SHELTER/NEST
SKYIR <- C3SHEL * ((100 - SHADE) / 100) * (TSKY + 273.15) ^ 4 + C3SHEL * ((SHADE) / 100) * (TA + 273.15) ^ 4
GRDIR <- C4SHEL * (TGRD + 273.15) ^ 4
SHELIRIN = SKYIR + GRDIR

QRESP <- tZBRENT.out[2]
QSUM <- 0
QRESP <- 0
ZBRENT.in <- c(TA, VEL, SHELSOLR, SHELIRIN, BP, ELEV, D_NEST, RONEST, RFUR, TKNEST, QSUM, QRESP, EMISHEL, NGSHEL, SHELEN, THCONW, SHELAREA)

# FIND BRACKETING VALUES FOR GUESSING
ZBRAC <- ZBRAC_SHELTER(X1,X2,ZBRENT.in)
# DO THE GUESSING AND GET A SOLUTION FOR OUTER NEST TEMPERATURE
# INNER NEST TEMPERATURE WILL BE THE ENVIRONMENTAL CONDITIONS FOR THE NEXT ITERATION
# FOR GUESSING THE ANIMAL'S/HOMINID'S ENERGY BALANCE
X1 <- ZBRAC[1]
X2 <- ZBRAC[2]
X = ZBRENT_SHELTER(X1, X2, TOL1, ZBRENT.in)
# DEFINE X - IT IS ToNEST
ToNEST = X

# COMPUTING NEST INTERIOR WALL TEMPERATURE, Ti.
# THESE SOLUTIONS ARE FOR NO HEAT GENERATION IN THE WALL
if(NESTYP == 1){
 TINEST <- (QGENET * SHELEN) / (THCONW * SHELAREA) + ToNEST
}
if(NESTYP == 3 | NESTYP == 4){
 # CYLINDER: Q=((2*PI*K*L)/(LN(Ro/Ri)))*(Ti - To)
 TINEST <- (QGENET * (log(RONEST / RINEST))) / (2 * pi * THCONW * SHELEN) + TONEST
}
if(NESTYP == 2 | NESTYP == 5 | NESTYP == 6){
 # HOLLOW SPHERE: Q = ((4*PI*Ri*Ro*K)/(Ro-Ri))*(Ti - To)
 TINEST <- (QGENET * (RONEST - RINEST)) / (4 * pi * RONEST * RINEST * THCONW) + ToNEST
}
```
