...
* ```soil_temperature_lowshade``` Hourly predictions of the soil temperatures under the minimum specified shade
* ```soil_temperature_highshade``` Hourly predictions of the soil temperatures under the maximum specified shade
...
The first two rows of the output tables ```soil_temperature_lowshade``` and ```soil_temperature_highshade``` look like this 

```{r, echo=FALSE, results='asis', message=FALSE, warnings=FALSE}
knitr::kable(head(micro[, 1:12], 2), digits = 2)
```
...
1. DOY - day of year
1. TIME - time of day (mins)
1. Columns 3-12 "0cm" ... - soil temperatures at each of the 10 specified depths

Have a look at yours:
```
soil_temperature_lowshade <- as.data.frame(micro$soil_temperature_lowshade) # put the soil_temperature_lowshade result into a variable 'soil_temperature_lowshade'
head(soil_temperature_lowshade,24) # show the first 24 rows, i.e. the first day

soil_temperature_highshade<-micro$soil_temperature_highshade # put the metout result into a variable 'soil_temperature_highshade'
head(soil_temperature_highshade,24) # show the first 24 rows, i.e. the first day
```
...
soil_temperature_lowshade <- as.data.frame(micro$soil_temperature_lowshade) # get the soil_temperature_lowshade data
minshade <- micro$minshade[1] # get the value for minimum shade
with(subset(soil_temperature_lowshade, day_of_year==196 | day_of_year==349), {xyplot(depth_0cm + depth_2.5cm + depth_5cm + depth_10cm + depth_15cm + depth_20cm + depth_30cm + depth_50cm + depth_100cm + depth_200cm ~ time | as.factor(day_of_year), ylim = c(-20, 70), xlab = "Time of Day (min)", ylab = "Soil Temperature (deg C)", auto.key = list(columns = 5), as.table = TRUE, type = "b", main = paste(minshade,"% shade"))})
```
...
The output tables ```soil_moisture_lowshade``` and ```shadmoist```, which previously were zero, will now have soil moisture values (m3/m3) in them: 
```{r, message=FALSE, warnings=FALSE, echo=FALSE, results='asis'}
knitr::kable(head(micro$soil_moisture_lowshade[, 1:9], 2))
...
```{r, message=FALSE, warnings=FALSE, echo=FALSE, results='asis'}
knitr::kable(head(micro$soil_humidity_lowshade[, 1:9], 2))
...
```{r, message=FALSE, warnings=FALSE, fig.width=7, fig.height=6}
soil_moisture_lowshade <- as.data.frame(micro$soil_moisture_lowshade) # get the minimum shade soil moisture output
minshade <- micro$minshade[1] # get the value for minimum shade
# append dates
days <- rep(seq(1, 12), 24)
days <- days[order(days)]
dates <- days + soil_moisture_lowshade$time / 60 / 24 - 1 # dates for hourly output
soil_moisture_lowshade<-cbind(dates, soil_moisture_lowshade)
for(i in 1:10){
  if(i == 1){
    plot(soil_moisture_lowshade[, i + 3] ~ soil_moisture_lowshade[, 1], ylim = c(0, 0.5), xlab = "Month",  ylab = "Soil Moisture (% vol)", col = i, type = "l", main = paste("soil moisture, ", minshade, "% shade"))
  }else{
    points(soil_moisture_lowshade[, i + 3] ~ soil_moisture_lowshade[, 1], col = i, type = "l")
  }
}
```
...
for(i in 1:10){
  if(i==1){
    plot(soil_moisture_lowshade[,i+3]~soil_moisture_lowshade[,1], ylim=c(0,0.5),xlab = "Month", ylab = "Soil Moisture (%
...
days<-days[order(days)]
dates<-days+soil_moisture_lowshade$time/60/24/(12) # dates for hourly output
soil_moisture_lowshade<-cbind(dates,soil_moisture_lowshade)
for(i in 1:10){
  if(i==1){
    plot(soil_moisture_lowshade[,i+3]~soil_moisture_lowshade[,1], ylim=c(0,0.5),xlab = "Year", ylab = "Soil Moisture (%
      vol)",col=i,type = "l",main=paste("soil moisture, ",minshade,"% shade"))
  }else{
    points(soil_moisture_lowshade[,i+3]~soil_moisture_lowshade[,1],col=i,type = "l")
  }
...
for (i in 1:10) {
    if (i == 1) {
        plot(soil_moisture_lowshade[, i + 3] ~ soil_moisture_lowshade[, 1], ylim = c(0, 0.5), xlab = "Day of Year", ylab = "Soil Moisture (% vol)", col = i, type = "l", main = paste("soil moisture, ", minshade, "% shade"))
    } else {
        points(soil_moisture_lowshade[, i + 3] ~ soil_moisture_lowshade[, 1], col = i, type = "l")
    }
}
...
```{r, message=FALSE, warnings=FALSE, fig.width=7, fig.height=6}
soil_temperature_lowshade <- as.data.frame(micro$soil_temperature_lowshade)  # get the minimum shade soil temperature output
minshade <- micro$minshade[1]  # get the value for minimum shade
# append dates
days <- rep(seq(1, timeinterval), 24)
days <- days[order(days)]
dates <- days + soil_temperature_lowshade$time/60/24 - 1  # dates for hourly output
soil_temperature_lowshade <- cbind(dates, soil_temperature_lowshade)
for (i in 1:10) {
    if (i == 1) {
        plot(soil_temperature_lowshade[, i + 3] ~ soil_temperature_lowshade[, 1], ylim = c(-20, 70), xlab = "Day of Year", ylab = "Soil Temperature (deg C)", col = i, type = "l", main = paste("soil temperature, ", minshade, "% shade"))
    } else {
        points(soil_temperature_lowshade[, i + 3] ~ soil_temperature_lowshade[, 1], col = i, type = "l")
    }
}
```
...
soil_temperature_lowshade <- as.data.frame(micro$soil_temperature_lowshade)[(365 * 24 + 1):(365 * 24 * nyears), ]  # get the minimum shade soil temperature output, discarding the first year
...
for (i in 1:10) {
    if (i == 1) {
        plot(soil_temperature_lowshade[, i + 3] ~ soil_temperature_lowshade[, 1], ylim = c(-20, 70), xlab = "Day of Year", ylab = "Soil Temperature (deg C)", col = i, type = "l", main = paste("soil_temperature_lowshade  temperature, ", minshade, "% shade"))
    } else {
        points(soil_temperature_lowshade[, i + 3] ~ soil_temperature_lowshade[, 1], col = i, type = "l")
    }
}
...
soil_temperature_lowshade<-as.data.frame(micro) # get the minimum shade soil temperature output first year
minshade<-micro$minshade[1] # get the value for minimum shade
# append dates
days<-rep(seq(1,timeinterval),24)
days<-days[order(days)]
...
for(i in 1:10){
  if(i==1){
    plot(soil_temperature_lowshade[,i+3]~soil_temperature_lowshade[,1], ylim=c(-20,70),xlab = "Day of Year", ylab = "Soil Temperature (deg
...
Campbell, G. S., J. D. J. Jungbauer, W. R. Bidlake, and R. D. Hungerford. 1994. Predicting the effect of temperature on soil_temperature_lowshade thermal conductivity. Soil Science 158:307-313.

Campbell, G. S., and J. M. Norman. 1998. Environmental Biophysics. Springer, New York.

Kearney, M. R., A. Shamakhy, R. Tingley, D. J. Karoly, A. A. Hoffmann, P. R. Briggs, and W. P. Porter. 2014. Microclimate modelling at macro scales: a test of a general microclimate model integrated with gridded continental-scale soil_temperature_lowshade and weather data. Methods in Ecology and Evolution 5:273-286.
