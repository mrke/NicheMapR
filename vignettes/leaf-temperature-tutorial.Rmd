...
f_open <- rep(1, nrow(micromet_lowshade))
f_open[micromet_lowshade$zenith_angle == 90] <- 0 # close stomata when the sun is set
...
T_air_1.2m <- micromet_lowshade$air_temperature_reference
T_air_1cm <- micromet_lowshade$air_temperature_local
...
Next we make the same calculation with the 'ectoR_devel' function, extracting the relevant microclimate variables from the micromet_lowshade and soil output tables and matching the substrate solar absorptivity with that assumed in the microclimate model run. This is done in a loop and is an order of magnitude slower than using the 'ectotherm' function.

```{r, fig.width=7, fig.height=5, fig.show = "hold", message=FALSE, warnings=FALSE}
TAs <- micromet_lowshade$air_temperature_local
TGRDs <- soil$0cm
TSKYs <- micromet_lowshade$sky_temperature
VELs <- micromet_lowshade$wind_speed_local
RHs <- micromet_lowshade$relative_humidity_local
QSOLRs <- micromet_lowshade$solar_radiation
Zs <- micromet_lowshade$zenith_angle
alpha_sub <- (1 - micro$REFL)

n <- nrow(micromet_lowshade)
...
for(i in 1:n){
  ectoR.out <- ectoR_devel(leaf = leaf,
                           ...
                           TSKY = TSKYs[i],
                           VEL = VELs[i],
                           relative_humidity_reference = RHs[i],
                           QSOLR = QSOLRs[i],
                           Z = Zs[i],
                           pres = P_a[i],
                           PDIF = PDIFs[i])
...
T_leaf_Campbell <- unlist(lapply(1:length(TAs), function(x){leaf_temperature(
  ...
  relative_humidity_reference = RHs[x], # relative humidity from microclimate model, %
...
for(i in 1:length(TAs)){
  enviro_par <- make_enviropar(
    replace = list(
      relative_humidity_reference = set_units(RHs[i] / 100, "1"),
...
soil_water_potential_lowshade <- as.data.frame(micro$soil_water_potential_lowshade)
plant <- as.data.frame(micro)
```
