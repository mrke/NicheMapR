...
C     COOLER TEMPERATURES AND HIGHER WIND SPEEDS.
      ...
      DOUBLE PRECISION TCOND,SHTCOND,CONV_ENHANCE,G_VS_AB,G_VS_AD
      DOUBLE PRECISION MR_4
      ...
      INTEGER NODNUM,NON,POSTUR,SHDBURROW,WINGCALC,WINGMOD,LEAF,FLYHIGH
     ...
     &,WINGCALC,WINGMOD      
      ...
      IF((LIVE.EQ.1).AND.(TC.LT.TMINPR).AND.(ASIL.NE.ASILN))THEN
...
C      ON THE SURFACE, SUN IS UP. DIURNAL BEHAVIOURAL THERMOREGULATION WHAT'S THE CORE TEMPERATURE, TOO HOT OR TOO COLD?
       IF(TC.GT.TPREF)THEN
        IF((POSTUR.NE.2).AND.(LIVE.EQ.1).AND.
     &   (ASIL.GT.((ASILN+ASILP)/2.)).AND.   
     &   (SHADE.EQ.0).AND.(TC.GT.TMAXPR).AND.(FLYTEST.EQ.0))THEN
...
C         INCREASE THE SHADE (CALL SHADEADJUST) & RECALCULATE LOCAL ENVIRONMENT (CALL ABOVEGROUND)
        ...
        IF(TPREF .LE. TMAXPR)THEN
         ...
         ENDIF
        ENDIF
        IF((FLYMETAB.GT.0.).AND.(TC.GT.TMAXPR))THEN
         FLYMETAB=0. ! TOO HOT TO FORAGE WITH EXTRA METABOLIC LOAD OF ACTIVITY
         CALL ABOVEGROUND
         RETURN
        ENDIF
