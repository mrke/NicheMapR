...
C     GIVEN GAS CONCENTRATIONS, PRESSURE, RESPIRATION RATE AND HUMIDITY
      ...
      DOUBLE PRECISION FLUID,FOODLEVELCUR,FOODWATERCUR,G,GEVAP,GH2OMET
      DOUBLE PRECISION GH2OMET_INIT,GMASS,H2O_BALPAST,HTOVPR,KGEVAP,MLO2
      DOUBLE PRECISION MLO2_INIT,MR_1,MR_2,MR_3,N2GAS,N2MOL1,N2MOL2
      DOUBLE PRECISION NWASTE,O2GAS,O2MOL1,O2MOL2,O2MOLC,O2STP,PCTCO2
      DOUBLE PRECISION PCTN2,PCTO2,PFEWAT,PHI,PHIMAX,PHIMIN,PO2,PTCOND
      DOUBLE PRECISION PTCOND_ORIG,QCOND,QCONV,QIRIN,QIROUT,QMETAB,QRESP
      DOUBLE PRECISION QSEVAP,QSOLAR,R,REFPO2,RELHUM,RELXIT,RGC,RHO1_3
      DOUBLE PRECISION RPCTCO2,RPCTN2,RPCTO2,RQ,RW,SHP,SIDEX,SIG
      DOUBLE PRECISION SUBTK,T_A,T_AH,T_AL,T_H,T_L,T_REF,TA,TAIR,TBASK
      DOUBLE PRECISION TC,TCORR,TDIGPR,TEMERGE,TLUNG,TMAXPR,TMINPR,TPREF
      ...
      DOUBLE PRECISION XCALC,XTRY,TOTGAS,VAIR,VCO2,CO2MOL2,CONV_ENHANCE
      DOUBLE PRECISION WEYES,WRESP,WCUT,AEFF,CUTFA,HD,PEYES,SKINW
      DOUBLE PRECISION SKINT,HC,CONVAR,PMOUTH,PANT,PANTMAX,EGGPTCOND,POT
      DOUBLE PRECISION G_VS_AB,G_VS_AD,MR_4,QAIR
      
      INTEGER DEB1,GEOMETRY,IHOUR,NODNUM,WINGMOD,WINGCALC,ARRHEN,LEAF
...
C     NOTE THAT THERE IS NO RECOVERY OF MOISTURE ASSUMED IN THE NOSE
...
C     CHECK FOR TOO LARGE OR SMALL CORE TEMP
     ...
     &EXP(T_AL/(273.15+TC)-T_AL/T_L)+EXP(T_AH/T_H-T_AH/(273.15+TC))))
      ENDIF
...
C     HEAT EXCHANGE DUE TO TEMPERATURE AIR THAT ENTERS THE LUNG
      CALL WETAIR(TAIR,WB,RELHUM,DP,BP,E,ESAT,VD,RW,TVIR,TVINC,
     * DENAIR,CP,WTRPOT)
