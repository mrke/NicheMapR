...
C     DDSUB = DTCORE/DTIME
      ...
      DOUBLE PRECISION MR_3,NWASTE,O2MAX,O2MIN,PCTDIF,PCTEYE,PEYES,PHI
      DOUBLE PRECISION PHIMAX,PHIMIN,PI,PTCOND,PTCOND_ORIG,QCOND,QCONV
      DOUBLE PRECISION QGENET,QIN,QIRIN,QIROUT,QMETAB,QOUT,QRESP,QSEVAP
      DOUBLE PRECISION QSOL,QSOLAR,QSOLR,QST,R,R1,R2,REFTOL,RELHUM
      DOUBLE PRECISION RH,RHO1_3,RHREF,RHUM,RINSUL,RQ,RW,S2,SHADE
...
C     INITIALIZING FOR SUB. WETAIR
     ...
     * DENAIR,CP,WTRPOT)
     ...
     & ((SUBTK*AV)/(VOL**(1./3.)))*((GN*S2)/(2*FLSHCOND)+TSUBST))
      ELSE
       IF(POND.EQ.0)THEN
        R2=(ALENTH/2)**2
        J=(1/(AMASS*SPHEAT))*((QSOLAR+QGENET+CONVAR*(((GN*R2)/(4*
     &  FLSHCOND))*(HC+HR)+HC*TA+HR*TRAD)+AEFF*(HD*HTOVPR*VDAIR))+
     &  ((SUBTK*AV)/R2)*((GN*R2)/(4*FLSHCOND)+TSUBST))
       ELSE
        R2=(CONTDEP/1000/2)**2
        J=(1/(AMASS*SPHEAT))*((QSOLAR+QGENET+CONVAR*(((GN*R2)/(4*
     &  FLSHCOND))*(HC+HR)+HC*TA+HR*TRAD)+AEFF*(HD*HTOVPR*VDAIR))+
...
C	  CONTAINER/POND CALCULATIONS
      IF(CONTH.GT.0)THEN
       ...
       ENDIF
       CONTDEP=VOLUMELEFT/(PI*(CONTW/2./100.)**2.)*1000.
