...
C     ORIGINALLY DEVELOPED FOR A GARTER SNAKE, THAMNOPHIS ELEGANS
     ...
     & PAS,PBS,PCS,PDS,PGS,PJS,PMS,PRS,WETFOOD
      DOUBLE PRECISION TLUNG,DELTAR,EXTREF,RQ,MR_1,MR_2,MR_3
...
C      ABOVE GROUND - USE WOOD THERMAL CONDUCTIVITY FOR SUBSTRATE
       SUBTK2=0.10 ! CHANGE THIS TO SOIL SURFACE CONDUCTIVITY
      ELSE
C      USE STANDARD SOIL THERMAL CONDUCTIVITY
       SUBTK2=SUBTK ! CHANGE THIS TO SOIL CONDUCTIVITY AT CHOSEN DEPTH
      ENDIF

      IF((DEB1.EQ.1).AND.(STAGE.LT.1).AND.(VIVIPAROUS.EQ.0).AND.
     & (AMASS.GT.0.))THEN
       QCOND=AV*(SUBTK2/MIN(ASEMAJR,BSEMINR,CSEMINR))*(TSKIN-TSUBST) ! CONDUCTION HEAT FLOW
      ELSE
