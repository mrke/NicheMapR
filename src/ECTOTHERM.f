...
C     MKS SYSTEM OF UNITS (J, M, KG, S, C, K, PA)
     ...
     & INTENT(IN) :: S_INSTAR1
      ...
      DOUBLE PRECISION FOODMIN,FOODWATER,FOODLEVELCUR,FOODWATERCUR,FUN
      DOUBLE PRECISION FUNCT,G,GEVAP,GH2OMET,GH2OMET_INIT,GUTFILL
      DOUBLE PRECISION GUTFREEMASS,GUTFULL,H_AREF,H2O_BAL,H2O_BAL_HR
      ...
      DOUBLE PRECISION SIDEX,SIG,SKINT,SKINW,SPHEAT,STAGE
...
C     ****************** BEGINING OF I/O *********************
      ...
      ALLOCATE (FOODWATERS(NN*24),FOODLEVELS(NN*24))
...
C       RESETTING INPUT PARAMETERS
        ...
        ELSE
         FOSORIAL='N'
        ENDIF
        !H2O_BALPAST=ECTOINPUT(59) !WAS NOFOOD
        RAINTHRESH=ECTOINPUT(61)
       ...
       IF(DEAD.EQ.1)THEN ! ANIMAL DIED, REINITIALISE
        ...
        !EH_BABY_INIT=0.
        MLO2_INIT=10.**(MR_3*TC)*MR_1*(AMASS*1000.)**MR_2 ! REGRESSION-BASED
...
C     SETTING DAY OF YEAR COUNTER (FOR TRANSIENT CALCULATIONS)
      ...
      IF(DAYCOUNT.EQ.1)THEN ! SET INPUT VARIABLES
       ...
       ENDIF
       !H2O_BALPAST=ECTOINPUT(59) !WAS NOFOOD
       !LIQ_INIT=H2O_BALPAST !CHECK THIS WORKS - PROBABLY NEEDS ANOTHER SPECIFIC INPUT
...
C      IYEAR=DEBLAST1(1)
       ...
       MLO2_INIT=10.**(MR_3*TC)*MR_1*(AMASS*1000.)**MR_2 
       GH2OMET_INIT=0.0D+00
       DEBQMET_INIT=0.0D+00
       IF((DEB1.EQ.1).AND.(STAGE.EQ.0.))THEN
        PSI_INIT=PSI_EGG
...
C     &    (1-EGGDRYFRAC)+V_INIT*(1-D_V))/1000.
        ...
        ELSE
         ...
         LIQ_INIT=(((V_INIT*E_INIT)/MU_E)*W_E)/D_V*(1.-D_V)/1000.
        ENDIF
...
C     SKIP OVER CHECKS IF CORE TEMPERATURE OK & SOLUTION WITHIN TOLERANCE
     ...
     &.AND.(DEB1.EQ.1).AND.(STAGE.GT.DBLE(STAGES-2)).AND.(TDIGPR.LT
     &.TPREF).AND.(Z(IHOUR).LT.90.)))THEN
...
C          HASN'T YET TRIED FLYING
     ...
     &.AND.(STAGE.GT.DBLE(STAGES-2)).AND.(Z(IHOUR).LT.90).AND
...
C ---  CALL OF THE SUBROUTINE DOPRI5
     ...
     &  WORK,LWORK,IWORK,LIWORK,RPAR,IPAR,IDID,VSTI)
      ...
      ENDIF
      PSI_BODY=PSI(IHOUR)
      CUMWATER=LIQ(IHOUR)*1000.-INITH2O ! CUMULATIVE WATER IN/OUT OF EGG/BODY FROM LIQUID AND VAPOUR EXCHANGE
      LIQDIF=LIQ(IHOUR)-LIQPAST
      LIQPAST=LIQ(IHOUR)
