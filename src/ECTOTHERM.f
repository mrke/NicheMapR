     ...
     &,SHADHUMID1,DEP1,RAINFALL1,DEBMOD1,DEBLAST1,FOODWATERS1
     &,FOODLEVELS1,WETLANDTEMPS1,WETLANDDEPTHS1,GLMTEMP1,GLMO21,GLMSALT1
     &,GLMPH1,GLMFOOD1,ARRHENIUS1,THERMAL_STAGES1,BEHAV_STAGES1
...
C     THERE ARE FOUR DEB MODELS - THE STANDARD MODEL (METAB_MODE=0), THE ABJ ACCELERATION MODEL 
...
C     MKS SYSTEM OF UNITS (J, M, KG, S, C, K, PA)
      ...
      USE AACOMMONDAT
      IMPLICIT NONE
      EXTERNAL FUN,FUNWING,DSUB2,JAC
     ...
     & MINSHADES1
      DOUBLE PRECISION, DIMENSION(INT(NN2)*24,27),INTENT(INOUT) ::
     &ENVIRON1
     ...
     & GLMTEMP1,GLMO21,GLMSALT1,GLMPH1,GLMFOOD1
      DOUBLE PRECISION, DIMENSION(INT(CEILING(NN2/365.)),45),
     ...
     & INTENT(IN) :: S_INSTAR1
      ...
      DOUBLE PRECISION BREEDTEMPTHRESH,BREF,CAUSEDEATH,CAVE,CLUTCHA
      ...
      DOUBLE PRECISION DRUNK,DRYFOOD,DTIME,DWIN,DWTR,E_BABY,E_BABY_INIT
      DOUBLE PRECISION E_BABY1,E_EGG,E_G,E_H,E_H_INIT,E_H_START,E_HB
      DOUBLE PRECISION E_HE,E_HJ,E_HP,E_HPUP,E_HPUP_INIT,E_INIT
      DOUBLE PRECISION E_INIT_BABY,E_M,ECTOINPUT,ECTOINPUT1,ED
      DOUBLE PRECISION EGGDRYFRAC,EGGSHSOI,EGGSOIL,EH_BABY,EH_BABY_INIT
      ...
      DOUBLE PRECISION EPUP,EPUP_INIT,ER,ERRTES,ES,ES_INIT,ESM,ETAO
      ...
      DOUBLE PRECISION FOODMIN,FOODWATER,FOODWATERCUR,FTRANSAR,FUN,FUNCT
      ...
      DOUBLE PRECISION HDFORC,HDFREE,HRSACT,HS,HS_INIT,HSHSOI,HSOIL
      DOUBLE PRECISION JM_JO,K_JREF,KAP_R,KAP_X,KAP_X_P,KAPPA,L_B,L_J
      DOUBLE PRECISION L_J2,L_W,L_WREPRO,LAMBDA,LAT,LENGTHDAY
      DOUBLE PRECISION LENGTHDAYDIR,LONGEV,MA,MAX_PCTDESI,MAXMASS
      DOUBLE PRECISION MAXSHADE,MAXSHD,MAXTC,MH,MI,MINCLUTCH,MINED
      DOUBLE PRECISION MINTC,MINWATER,MINWATER2,MINWATERLOW,MLO2
      ...
      DOUBLE PRECISION NEWDEP,NTRY1,NWASTE,O2FLUX,O2GAS,O2MAX,O2MIN
      DOUBLE PRECISION O2WATERS,OBJDIS,OBJL,ORIG_CLUTCHSIZE,ORIG_ESM
      DOUBLE PRECISION P_AM_ACC,P_AM_REF,P_B_PAST,P_MREF,P_XM_ACC
      DOUBLE PRECISION P_XMREF,PCTDESI,PCTDIF,PCTEYE,PEYES,PFEWAT,PHI
      ...
      DOUBLE PRECISION Q,Q_INIT,QCOND,QCONV,QIRIN,QIROUT,QMETAB,QRAD
      DOUBLE PRECISION QRESP,QSEVAP,QSOL,QSOLAR,QSOLR,QSOLRF,QUIT,R,R0
      DOUBLE PRECISION R1,RAINDRINK,RAINFALL,RAINMULT,RAINTHRESH,REFLSH
      DOUBLE PRECISION REFSHD,REFTOL,RELHUM,REPRO,RH,RHO1_3,RHREF,RINSUL
      DOUBLE PRECISION RMAX,RQ,RTOL,RWORK,S_G,S_J,SALWATER,SALWATERS
      DOUBLE PRECISION SHADE,SHD,SHDACT,SHDSOIL,SHP,SIDEX,SIG,SKINT
      DOUBLE PRECISION SKINW,SPARE1,SPARE2,SPARE3,SPARE4,SPARE5,SPHEAT
      DOUBLE PRECISION STAGE,STAGE_REC,SUBTK,SUNACT,SUNSOIL,SURVIV
      DOUBLE PRECISION SURVIV_INIT,T,T_A,T_AH,T_AL,T_H,T_L,T_REF,TA
      DOUBLE PRECISION TAIREF,TALOC,TANNUL,TBASK,TC,TC_OLD,TCINIT
      DOUBLE PRECISION TCORES,TCPAST,TDIGPR,TEIN,TEMERGE,TESTX,TEVP,TIME
      DOUBLE PRECISION TIMEND,TLUNG,TMAXPR,TMET,TMINPR,TNRG,TOBJ,TOTLEN
      DOUBLE PRECISION TOUT,TPREF,TPRINT,TQSOL,TR,TRANS1,TRANSAR
      ...
      DOUBLE PRECISION WQSOL,WRESP,X,X_FOOD,X1,X2,XTRY,XWATERS,Y,Y_EV_L
      ...
      DOUBLE PRECISION YDSTG8,YEARFRACT,YEAROUT,YFEC,YFECES,YFIT,YFOOD
      ...
      DOUBLE PRECISION YO2,YOVIPSURV,YSURV,YTMAX,YTMIN,Z,ZBRENT,ZEN
      ...
      INTEGER GOODSOIL,HCT,HOURCOUNT,HRCALL,I,I2,IDAY,IHOUR,II1,II2,II3
      INTEGER II4,II5,II6,INTMETHOD,INTNUM,INWATER,IOPT,ISS,ISTATE,IT
      INTEGER ITASK,ITEST,ITOL,IWORK,IYEAR,J,JP,JULSTART,LIVE,LIW,LRW
      INTEGER METAB_MODE,METAMORPH,MF,MICRO,MICROF,MICROF2,MICROS
      INTEGER MICROS2,MICROYEAR,MINNODE,MINWDEP,NA,NALT,NDAYY,NEQ,NM,NN
      INTEGER NODNUM,NON,NQ,NS,NSIMDAY,NTOBJ,NTRY,NTSUB,NYEAR,NZ
      INTEGER PHOTODIRF,PHOTODIRS,PHOTOFINISH,PHOTOSTART,POND,POSTUR
      INTEGER PREGNANT,RESET,SCENAR,SCENTOT,SHDBURROW,SOILNODE,STAGE3
      INTEGER STAGES,STARTDAY,STARVING,TESTER,TRAN,TRANCT,TRANNY
      INTEGER VIVIPAROUS,WARMSIG,WETMOD,WINGCALC,WINGMOD,WRITECSV
      ...
      DIMENSION ATSOIL(25,10),BREEDVECT(24),CUMBATCH(24),CUMREPRO(24)
      DIMENSION CUSTOMGEOM(8),DEBFIRST(13),DEBLAST1(13),DEBMOD1(94)
      DIMENSION DEBQMET(24),DEP1(10),DEPSEL(25),DRYFOOD(24),E_BABY1(24)
      DIMENSION E_H(24),E_HPUP(24),ECTOINPUT(131),ECTOINPUT1(131),ED(24)
      DIMENSION EGGSHSOI(25),EGGSOIL(25),EH_BABY1(24),ENARY1(25)
      ...
      DIMENSION ETAO(4,3),FAECES(24),FOOD(50),FTRANSAR(25),GH2OMET(24)
      DIMENSION HS(24),HSHSOI(25),HSOIL(25),INACTIVE(25),IWORK(200)
      DIMENSION JM_JO(4,4),L_W(24),MLO2(24),MSHSOI(25),MSOIL(25)
      DIMENSION NTRY1(24),NWASTE(24),O2WATERS(20),PHWATERS(20)
      DIMENSION PSHSOI(25),PSOIL(25),Q(24),QSOL(25),REPRO(24),RH(25)
      DIMENSION RHREF(25),RWORK(200),SALWATERS(20),SHD(25),SHP(3)
      DIMENSION STAGE_REC(25),SURVIV(24),TALOC(25),TCORES(25),TIME(25)
      DIMENSION TRANSAR(5,25),TREF(25),TSHLOW(25),TSHOIL(25)
...
C     ****************** BEGINING OF I/O *********************
      ...
      ALLOCATE (WETLANDDEPTHS(NN*24),WETLANDTEMPS(NN*24))
...
1111  FORMAT(26(A8,A1),A8)
1112  FORMAT(20(A13,A1),A13)
1113  FORMAT(13(A8,A1),A8)
1114  FORMAT(20(A13,A1),A13)
...
C       RESETTING INPUT PARAMETERS
        ...
        SPARE1=ECTOINPUT(18) ! WAS TIMBAS
...
C     SETTING DAY OF YEAR COUNTER (FOR TRANSIENT CALCULATIONS)
      ...
      IF(DAYCOUNT.EQ.1)THEN ! SET INPUT VARIABLES
       ...
       SPARE1=ECTOINPUT1(18) ! WAS TIMBAS
       ...
       SKINW=ECTOINPUT1(30)
       PEYES=ECTOINPUT1(31)
       SPARE2=ECTOINPUT1(32) ! WAS XBAS
       EXTREF=ECTOINPUT1(33)
       TPREF=ECTOINPUT1(34)
       TDIGPR=TPREF
       PTCOND=ECTOINPUT1(35)
       PTCOND_ORIG = PTCOND
       SKINT=ECTOINPUT1(36)
       O2GAS=ECTOINPUT1(37)
...
C    CHECK IF BUCKET MODEL HAS RUN (OR IF WET0D WAS RUN) AND, IF SO, SWITCH OFF POND
     ...
     &  (ECTOINPUT1(116).EQ.1))THEN
       ...
       SPARE5=INT(ECTOINPUT1(73)) !WAS MONTHLY
       ...
       CONTWET=ECTOINPUT1(123)
       WARMSIG=INT(ECTOINPUT1(124))
...
C      OTHER DEB-RELATED PARAMETERS
       ...
       JM_JO(1,3)=DEBMOD1(62)
       JM_JO(2,3)=DEBMOD1(63)
       JM_JO(3,3)=DEBMOD1(64)
       JM_JO(4,3)=DEBMOD1(65)
       E_EGG=DEBMOD1(66)
       KAP_X_P=DEBMOD1(67)
       PTUREA=DEBMOD1(68)
       PFEWAT=DEBMOD1(69)
...
c     INITIALISING FOR SUMMARY CALCULATIONS OF ANNUAL STATISTICS
...
233     CONTINUE
        IF(BURROWSHADE.EQ.0)THEN
         CURSOIL=SOIL1((IHOUR+24*(DAYCOUNT-1))-1,2+CURNOD)
        ELSE
         CURSOIL=SHADSOIL1((IHOUR+24*(DAYCOUNT-1))-1,2+CURNOD)
        ENDIF
        IF((TC_OLD.LT.TEMERGE)
     &   .OR.
     &   ((TC_OLD.GE.TEMERGE) ! POTENTIALY COULD EMERGE
     &     .AND.
     &     (WARMSIG.EQ.1) ! NEEDS A SIGNAL OF WARMING
     &     .AND.
     &     (CURNOD.GT.2) ! NOT RIGHT NEAR SURFACE
     &     .AND.
     &     (CURSOIL-TC_OLD.LT.0.1) ! NO SIGNAL
     &     .AND.
...
C      IF(POND_DEPTH.GT.(CONTREF*10./2.))THEN
       IF(POND_DEPTH.GT.0)THEN!CONTREF*10*0.1)THEN
        BURROW='N'
        IF(ECTOINPUT(51).EQ.1)THEN
         DAYACT='Y'
        ELSE
...
C       CHECK IF TOO COLD IN RETREAT SO THE ANIMAL NEEDS TO BASK
        IF((((WARMSIG.EQ.0).OR.(CURNOD.LE.2))
     &    .AND.
     &    (TC.GE.TDIGPR)
     &    .AND.
     &    (TC.GE.TEMERGE))
...
C       HAD THIS AT TSHSOI(4) FOR WESTERNS SWAMP TORTOISE
       ...
       ENDIF
       IF((AQUATIC.EQ.1).AND.(POND.EQ.1))THEN
C       HAD THIS AT 40. FOR WESTERN SWAMP TORTOISE
        ENARY8(IHOUR)= 0.0
       ELSE
        ENARY8(IHOUR)= 0.0
       ENDIF
...
C      CAVE/DEEP UNDERGROUND
       ...
       ENARY40(IHOUR)=100.0
C      FINAL ARRAY OF ENVIRONMENTAL CONDITIONS FOR DSUB2, TO BE WRITTEN LATER
       ENARY41(IHOUR)=QSOL(IHOUR)
       ENARY42(IHOUR)=Z(IHOUR)
       ENARY43(IHOUR)=TALOC(IHOUR)
       ENARY44(IHOUR)=VLOC(IHOUR)
...
C     ALMOST END OF HOURS LOOP
     ...
     & ENVIRON1(HCT,25),',',ENVIRON1(HCT,26),',',ENVIRON1(HCT,27)
        ENDIF
       ENDIF
      ENDIF
...
C        CORE TEMPERATURE
C        Y(1)=X;
C        YDOT(1)=DX/DT  COMPUTED IN DSUB2
...
C        IF A TRANSIENT IS INVOKED
         ...
         ELSE
          Y(1)=TALOC(1)
         ENDIF
         IF((TRANCT.GT.1).OR.(DDOY.GT.1))THEN
          Y(1)=TCPAST
         ENDIF
         QRAD=QIRIN-QIROUT
         IF(T.EQ.0.D0) THEN
          IT=1
          CALL OSUB2(T,Y,TRANCT,NDAYY)
         ENDIF
...
C        INTERNAL GEAR CALLS TO THE OUTPUT SUBROUTINE, OSUB2, PUT OUTPUT IN
...
C     *   IOPT,LRW,LIW,MF,TRANCT,NDAYY
         ...
         CALL LSODE(DSUB2,NEQ,Y,T,TOUT,ITOL,RTOL,ATOL,ITASK,ISTATE,
...
C    ****************NOW CONSTRUCTING FINAL ENVIRONMENTAL ARRAY**************
      DO 969 I=1,25
...
7891   CONTINUE
       ...
       DO 891 I=MICROS,MICROF
        ...
        IF((LIVE.EQ.1).OR.(CONTONLY.EQ.1).OR.(TRANNY.EQ.1))THEN
         IF((DEB1.EQ.1).AND.(METAB_MODE.GT.1))THEN
          CALL DEB_HOLO(I-24*(DAYCOUNT-1)) ! RUNNING THE HOLOMETABOLOUS INSECT VERSION
         ELSE
     ...
     &    T1(I,19),',',DEBOUT1(I,20),',',DEBOUT1(I,21)
         ENDIF
         IF(STAGE.EQ.0)THEN
          DEVTIME=DEVTIME+(1./24.)
         ENDIF
         IF((STAGE.GT.1).AND.(BIRTH.EQ.0))THEN
          ...
          BIRTHMASS=DEBOUT1(I,18)
         ENDIF
         ENBAL1(I+24*(IDAY-1),1)=DOY
         ENBAL1(I+24*(IDAY-1),2)=IYEAR
         ENBAL1(I+24*(IDAY-1),3)=DAYCOUNT
         ENBAL1(I+24*(IDAY-1),4)=I-24*(DAYCOUNT-1)
         ENBAL1(I+24*(IDAY-1),5)=QSOLAR
         ENBAL1(I+24*(IDAY-1),6)=QIRIN
         ENBAL1(I+24*(IDAY-1),7)=QMETAB
         ENBAL1(I+24*(IDAY-1),8)=QSEVAP
         ENBAL1(I+24*(IDAY-1),9)=QIROUT
         ENBAL1(I+24*(IDAY-1),10)=QCONV
         ENBAL1(I+24*(IDAY-1),11)=QCOND
         ENBAL1(I+24*(IDAY-1),12)=ENB
         ENBAL1(I+24*(IDAY-1),13)=NTRY
         IF(WRITECSV.EQ.2)THEN
          WRITE(II3,113)DOY,",",IYEAR,",",DAYCOUNT,",",
     &    ENBAL1(I+24*(IDAY-1),4),",",QSOLAR,",",QIRIN,",",
     &    QMETAB,",",QSEVAP,",",QIROUT,",",QCONV,",",QCOND,",",
     &    ENB,",",NTRY
         ENDIF
         ...
         ENDIF
         HCT=I+24*(IDAY-1)
         MASBAL1(I+24*(IDAY-1),1)=DOY
         MASBAL1(I+24*(IDAY-1),2)=IYEAR
         MASBAL1(I+24*(IDAY-1),3)=DAYCOUNT
         MASBAL1(I+24*(IDAY-1),4)=I-24*(DAYCOUNT-1)
         MASBAL1(I+24*(IDAY-1),5)=O2FLUX
         IF(DEB1.EQ.1)THEN
          MASBAL1(I+24*(IDAY-1),6)=CO2FLUX
          MASBAL1(I+24*(IDAY-1),7)=NWASTE(IHOUR)
          MASBAL1(I+24*(IDAY-1),8)=H2O_FREE
          MASBAL1(I+24*(IDAY-1),9)=GH2OMET(IHOUR)
          MASBAL1(I+24*(IDAY-1),10)=DRYFOOD(IHOUR)
          MASBAL1(I+24*(IDAY-1),11)=WETFOODFLUX
          MASBAL1(I+24*(IDAY-1),12)=FAECES(IHOUR)
          MASBAL1(I+24*(IDAY-1),13)=WETFAECESFLUX
          MASBAL1(I+24*(IDAY-1),14)=URINEFLUX
          MASBAL1(I+24*(IDAY-1),18)=H2O_BAL_HR
          MASBAL1(I+24*(IDAY-1),19)=H2O_BAL
         ELSE
          MASBAL1(I+24*(IDAY-1),6)=0
          ...
          MASBAL1(I+24*(IDAY-1),18)=0
          MASBAL1(I+24*(IDAY-1),19)=0
         ENDIF
         MASBAL1(I+24*(IDAY-1),15)=WRESP*3600
         MASBAL1(I+24*(IDAY-1),16)=WCUT*3600
         MASBAL1(I+24*(IDAY-1),17)=WEYES*3600
         IF(WRITECSV.EQ.2)THEN
          WRITE(II2,112)MASBAL1(HCT,1),",",MASBA
     &    L1(HCT,2),",",MASBAL1(HCT,3),",",MASBA
     &    L1(HCT,4),",",MASBAL1(HCT,5),",",MASBA
     ...
     &    ,",MASBAL1(HCT,18),",",MASBAL1(HCT,19)
         ...
         ENVIRON1(I+24*(IDAY-1),7)=ENARY41(I-24*(DAYCOUNT-1))
         ...
         ENVIRON1(I+24*(IDAY-1),11)=ENARY47(I-24*(DAYCOUNT-1))
         ENVIRON1(I+24*(IDAY-1),12)=ENARY46(I-24*(DAYCOUNT-1))
         ENVIRON1(I+24*(IDAY-1),13)=ENARY44(I-24*(DAYCOUNT-1))
         ENVIRON1(I+24*(IDAY-1),14)=ENARY45(I-24*(DAYCOUNT-1))
         ENVIRON1(I+24*(IDAY-1),15)=ENARY42(I-24*(DAYCOUNT-1))
         IF((AQUATIC.EQ.1).AND.(POND.EQ.0))THEN
          ENVIRON1(I+24*(IDAY-1),16)=ENVIRON1(I+24*(IDAY-1),14)
          ENVIRON1(I+24*(IDAY-1),17)=ENVIRON1(I+24*(IDAY-1),15)
         ELSE
          IF(CONTH.GT.0)THEN
           ENVIRON1(I+24*(IDAY-1),16)=CONTDEPTH
           ENVIRON1(I+24*(IDAY-1),17)=TRANSIENT(I-24*(DAYCOUNT-1))
C          POND_ENV(I+24*(IDAY-1),1)=DOY
          ELSE
           ENVIRON1(I+24*(IDAY-1),16)=0
           ENVIRON1(I+24*(IDAY-1),17)=0
          ENDIF
         ENDIF
         ENVIRON1(I+24*(IDAY-1),18)=LENGTHDAY
         ENVIRON1(I+24*(IDAY-1),19)=PHI
         ENVIRON1(HCT,20)=WINGTEMP
         ENVIRON1(HCT,21)=FLIGHT
         ENVIRON1(HCT,22)=FLYTIME
         ENVIRON1(HCT,23)=PO2WATER
     ...
     &    VIRON1(HCT,15),',',ENVIRON1(HCT,16),',',ENVIRON1(HCT
     &    ,17),',',ENVIRON1(HCT,18),',',ENVIRON1(HCT,19),',',
     &    ENVIRON1(HCT,20),',',ENVIRON1(HCT,21),',',
     &    ENVIRON1(HCT,22),',',ENVIRON1(HCT,23),',',ENVIRON1(HCT,
     &    24),',',ENVIRON1(HCT,25),',',ENVIRON1(HCT,26),',',
     &    ENVIRON1(HCT,27)
         ENDIF
        ENDIF
891    CONTINUE
...
C     6    POND DRY
      IF(COUNTDAY.EQ.CENSUS)THEN
       ...
       YEARSOUT1(IYEAR,43)=DBLE(YOVIPSURV)
       YEARSOUT1(IYEAR,44)=DBLE(YFIT)
       YEARSOUT1(IYEAR,45)=DBLE(DEATHSTAGE)
...
115   FORMAT(19(1F15.3,A),1F10.3)
      ...
      DEALLOCATE (POND_ENV,DAYDOY,RAINFALL2,WETLANDDEPTHS,MINSHADES,
     & MAXSHADES,WETLANDTEMPS,FOODWATERS,FOODLEVELS,XP,YP,DAY,TRANSIENT
     & ,ZP1,ZP2,ZP3,ZP4,ZP5,ZP6,ZP7,ZD1,ZD2,ZD3,ZD4,ZD5,ZD6,ZD7)
200   RETURN
      END
