...
C     MKS SYSTEM OF UNITS (J, M, KG, S, C, K, PA)
     ...
     & INTENT(IN) :: S_INSTAR1
      ...
      DOUBLE PRECISION E_H_START,E_HB,EGGSHP,E_HE,E_HJ,E_HO,E_M2,E_HP
      DOUBLE PRECISION E_HPUP,E_HPUP_INIT,E_INIT,E_INIT_BABY,E_M
      DOUBLE PRECISION ECTOINPUT,ECTOINPUT1,ED,EGGDRYFRAC,EGGSHSOI
...
C     SETTING OPTIONS FOR SHADE SELECTION WHEN INACTIVE
     ...
     &   .OR.((VIVIPAROUS.EQ.1).AND.(E_H(IHOUR).GE.E_HO))))THEN
...
C     CHECK TO SEE IF DEVELOPING EGG AND, IF IT IS, PUT IT IN THE GROUND
      IF((DEB1.EQ.1).AND.(POND.EQ.0))THEN
       IF(STAGE.EQ.0.)THEN
        IF(((VIVIPAROUS.EQ.1).AND.(E_H(IHOUR).LT.E_HO))
