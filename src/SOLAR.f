...
C     COMPUTES SOLAR RADIATION ABSORBED, EITHER FOR BODY OR (IF BUTTERFLY) WINGS
      ...
      INTEGER IHOUR,LIVE,MICRO,NM,WINGCALC,WINGMOD,POSTUR
...
C      DIRECT BEAM (NORMAL TO THE DIRECT BEAM)
       IF(POSTUR.EQ.3)THEN ! DON'T MAKE IT ORIENT TO SUN
        QNORM=QSOLR
       ELSE
        QNORM=(QSOLR/COS(ZEN))
       ENDIF
...
C       GREATER THAN THE SOLAR CONSTANT
       ...
       IF(ZENITH.GE.90.)THEN
        QNORM=0.000
       ENDIF
       QSDIR=ABSAN*ASIL*(1.00-PDIF)*QNORM
      ELSE
       QSDIR=0.00
       QNORM=0.00
      ENDIF
...
C      DIFFUSE COMPONENTS (SKY AND SUBSTRATE)
       ...
       IF(WINGMOD.EQ.1)THEN
        IF(PHI.LT.90)THEN
         QSSKY=0.
        ELSE
         QSSKY=ABSAN*FATOSK*(ATOT-AT/2.)*PDIF*QSOLR
        ENDIF
       ELSE
        QSSKY=ABSAN*FATOSK*(ATOT-AT/2.)*PDIF*QSOLR
       ENDIF
