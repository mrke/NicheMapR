...
C     AND DEBUGGING
     ...
     &BEHAV_STAGES2,WATER_STAGES2,ARRHENIUS3,ARRHENIUS32,TCOND2,
     &SHADTCOND2
      DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: RAINFALL3,RAINHR2,
     & PRESHR2,WETLANDDEPTHS2,WETLANDTEMPS2,FOODWATERS2,FOODLEVELS2
     &,MAXSHADES2,PCTWETS2,S_INSTAR2,MINSHADES2,PDIFS2,G_VS_ABS2,
     &G_VS_ADS2
      ...
      DOUBLE PRECISION E_H_START,E_HB,E_HE,E_HJ,E_HLAY,E_HP,E_HPUP
      DOUBLE PRECISION E_HPUP_INIT,E_INIT,E_INIT_BABY,E_M2,ECTOINPUT2,ED
      DOUBLE PRECISION EGGDRYFRAC,EGGSHP,EH_BABY
      ...
      DOUBLE PRECISION EPUP,EPUP_INIT,E_S,E_S_INIT,E_SM,ETAO,EXTREF,F12
      DOUBLE PRECISION F13,F14,F15,F16,F21,F23,F24,F25,F26,F31,F32,F41
      DOUBLE PRECISION F42,F51,F52,F61,FAECES,FATCOND,FATOBJ,FATOSB
      DOUBLE PRECISION FATOSK,FECUNDITY,FLSHCOND,FLTYPE,FLUID,FOOD
      DOUBLE PRECISION FOODLIM,FOODWATERCUR,FOODLEVELCUR,FUNCT,G,GEVAP
      DOUBLE PRECISION GH2OMET,GH2OMET_INIT,GUTFILL,GUTFREEMASS,GUTFULL
      DOUBLE PRECISION H_AREF,H2O_BALPAST,HALFSAT,HC,HD,HDFORC,HDFREE
      DOUBLE PRECISION HRN,HS,HS_INIT,HSHSOI,HSOIL,JM_JO,K_JREF,K_EL
      DOUBLE PRECISION K_EV,KAP,KAP_R,KAP_V,KAP_X,KAP_X_P,L_B,L_J,L_W
      DOUBLE PRECISION L_WREPRO,LAMBDA,LAT,LENGTHDAY,LENGTHDAYDIR,LONGEV
      DOUBLE PRECISION MAXCLUTCH,MAXMASS,MAXSHD,MINCLUTCH,MINED,MLO2
      DOUBLE PRECISION MLO2_INIT,MONMATURE,MONREPRO,MR_1,MR_2,MR_3
      DOUBLE PRECISION MSHSOI,MSOIL,MU_E,MU_N,MU_P,MU_V,MU_X,N2GAS
      DOUBLE PRECISION NEWCLUTCH,NEWDEP,NWASTE,O2FLUX,O2GAS
      DOUBLE PRECISION ORIG_CLUTCHSIZE,ORIG_E_SM,P_MREF,P_XMREF
      DOUBLE PRECISION PAS,PBS,PCS,PDS,PGS,PJS,PMS,PRS,MR_4
      DOUBLE PRECISION PANT,PANTMAX,PDIF,PEYES,PFEWAT,PHI
      ...
      DOUBLE PRECISION WQSOL,WRESP,X_FOOD,XTRY,YEAROUT2,Z,ZEN
      DOUBLE PRECISION ZFACT,ZSOIL,EGGPTCOND,POT,TCOND,SHTCOND
      DOUBLE PRECISION H_O,H_M,CONV_ENHANCE,G_VS_AB,G_VS_AD
      
      INTEGER AQUABREED,AQUASTAGE,ARRHEN,BATCH,BREEDACT,BREEDACTTHRES
      INTEGER BREEDING,BREEDVECT,CENSUS,COMPLETE,COMPLETION,COND_CLUTCH
      INTEGER CONTONLY,CONTYPE,COUNTDAY,CTKILL,CTMINCUM,CTMINTHRESH
      INTEGER DAYCOUNT,DEAD,DEADEAD,DEB1,DOY,EGGMULT,FOODMODE,GEOMETRY
      INTEGER GOODSOIL,I,IDAY,IHOUR,INTNUM,IT,IYEAR,J,JP,LEAF,LIVE
      INTEGER METAB_MODE,METAMORPH,MICRO,MINNODE,NM,NN3,NODNUM,NON,NS3
      INTEGER NYEAR,PHOTODIRF,PHOTODIRS,PHOTOFINISH,PHOTOSTART,PREGNANT
      INTEGER RAINHOUR,RESET,SCENAR,SOILNODE,STAGES,STARTDAY,TRANCT
      INTEGER VIVIPAROUS,WETMOD,WINGCALC,WINGMOD,STARVEMODE,ORIENT

      CHARACTER LABEL
      CHARACTER BURROW,DAYACT,CLIMB,CKGRSHAD,CREPUS,FOSORIAL,RAINACT
      CHARACTER NOCTURN,TRANST

      DIMENSION BREEDVECT(24),CUMBATCH(24),CUMREPRO(24),CUSTOMGEOM(8)
      DIMENSION DEBLAST2(15),DEBMOD2(103),DEBQMET(24),DEP2(10)
      DIMENSION DEPSEL(25),DRYFOOD(24),E_BABY1(24),E_H(24),E_HPUP(24)
      DIMENSION ECTOINPUT2(176),ED(24),EGGSHP(3),EH_BABY1(24),ENARY1(25)
      ...
      DIMENSION ENARY7(25),ENARY8(25),ENARY9(25),EPUP(24),E_S(24)
      DIMENSION ETAO(4,3),FAECES(24),FOOD(50),GH2OMET(24),HRN(25),HS(24)
      DIMENSION HSHSOI(10),HSOIL(10),JM_JO(4,4),L_W(24),MLO2(24)
      DIMENSION MSHSOI(10),MSOIL(10),NWASTE(24),PAS(24),PCS(24)
      ...
      DIMENSION TCOND(10),SHTCOND(10),H_O(4),H_M(4)
     ...
     & E_INIT_BABY,V_INIT_BABY,E_H_INIT,E_HB,E_HP,E_HJ,E_HLAY,E_SM,
...
C      NN3=1
      ...
      ALLOCATE ( TCOND2(24*NN3,12) )
      ALLOCATE ( SHADTCOND2(24*NN3,12) )
...
C     ALLOCATE (ZD5(NN3*25),ZD6(NN3*25),ZD7(NN3*25),DAY(NN3))
      ALLOCATE (WETLANDDEPTHS2(NN3*24),FOODLEVELS2(NN3*24))
      ALLOCATE (WETLANDTEMPS2(NN3*24),FOODWATERS2(NN3*24))
      ALLOCATE (PCTWETS2(NN3*24))
      ALLOCATE (G_VS_ABS2(NN3*24))
      ALLOCATE (G_VS_ADS2(NN3*24))
      ALLOCATE (PDIFS2(NN3*24))
...
29    CONTINUE
      ...
      OPEN(1,FILE='PCTWETS.CSV')
      READ(1,*)LABEL
      DO 51 I=1,NN3*24
      READ(1,*)LABEL,PCTWETS2(I)
51    CONTINUE
      CLOSE(1)
      
      OPEN(1,FILE='PDIFS.CSV')
      READ(1,*)LABEL
      DO 52 I=1,NN3*24
      READ(1,*)LABEL,PDIFS2(I)
52    CONTINUE
      CLOSE(1)      

      OPEN(1,FILE='G_VS_ABS.CSV')
      READ(1,*)LABEL
      DO 53 I=1,NN3*24
      READ(1,*)LABEL,G_VS_ABS2(I)
53    CONTINUE
      CLOSE(1)
      
      OPEN(1,FILE='G_VS_ADS.CSV')
      READ(1,*)LABEL
      DO 54 I=1,NN3*24
      READ(1,*)LABEL,G_VS_ADS2(I)
54    CONTINUE
      CLOSE(1)
...
37    CONTINUE
      ...
      OPEN(1,FILE='TCOND.CSV')
      READ(1,*)LABEL
      DO 47 I=1,NN3*24
      READ(1,*) (TCOND2(I,J),J=1,12)
47    CONTINUE
      CLOSE(1)
      
      OPEN(1,FILE='SHADTCOND.CSV')
      READ(1,*)LABEL
      DO 49 I=1,NN3*24
      READ(1,*) (SHADTCOND2(I,J),J=1,12)
49    CONTINUE
      CLOSE(1)      
...
45    CONTINUE
      ...
      OPEN(1,FILE='PRESHR.CSV')
      READ(1,*)LABEL
      DO 50 I=1,NN3*24
      READ(1,*)LABEL,PRESHR2(I)
50    CONTINUE
      CLOSE(1)
      
      CALL ECTOTHERM(NN3,NS3,ECTOINPUT2,METOUT2,SHADMET2,SOIL2,
     &SHADSOIL2,SOILMOIST2,SHADMOIST2,SOILPOT2,SHADPOT2,HUMID2
     &,SHADHUMID2,TCOND2,SHADTCOND2,DEP2,RAINFALL3,RAINHR2,PRESHR2
     &,DEBMOD2,DEBLAST2,FOODWATERS2,FOODLEVELS2,WETLANDTEMPS2
     &,WETLANDDEPTHS2,GLMTEMP2,GLMO22,GLMSALT2,GLMPH2,GLMFOOD2
     &,ARRHENIUS3,ARRHENIUS32,THERMAL_STAGES2,BEHAV_STAGES2
     &,WATER_STAGES2,NUTRI_STAGES2,MINSHADES2,MAXSHADES2,PCTWETS2,
     &G_VS_ABS2,G_VS_ADS2,PDIFS2
