...
C     IMPLEMENTATION OF LLANDRES ET AL. (2015) DEB MODEL FOR INSECTS
     ...
     &ATOL(1),VSTI(NDGL)
      ...
      DOUBLE PRECISION DSURVDT,DVDT,E_B_PRES,E_BABY,E_BABY_INIT,E_HJ
      DOUBLE PRECISION E_BABY1,E_EGG,E_G,E_H,E_H_INIT,E_H_PRES,E_H_START
      DOUBLE PRECISION E_HB,E_HE,E_HEGGS,E_HO,E_HP,E_HPUP,E_HPUP_INIT
      ...
      DOUBLE PRECISION E_SM,ETA_PA,ETAO,EXTREF,F,F12,F13,F14,F15,F16
      DOUBLE PRECISION F21,F23,F24,F25,F26,F31,F32,F41,F42,F51,F52,F61
      DOUBLE PRECISION FAECES,FATOSB,FATOSK,FECUNDITY,FLSHCOND,FLYMETAB
      DOUBLE PRECISION FLYSPEED,FLYTIME,FOOD,FOODLIM,FOODLEVELCUR
      ...
      DOUBLE PRECISION MAXCLUTCH,MAXMASS,MINCLUTCH,MINED,MLO2,MLO2_INIT
      ...
      DOUBLE PRECISION P_C,P_D,P_G,P_J,P_M,P_MREF,P_MV,P_R,MR_4
      ...
      DOUBLE PRECISION CNWASTE,CH2OMET,CUMWATER,REMH2O,LIQPAST
      ...
      INTEGER PHOTOFINISH,PHOTOSTART,PREGNANT,PREVDEAD,RAINHOUR,FLYHIGH
      INTEGER RESET,STAGES,STARTDAY,VIVIPAROUS,WAITING,WETMOD,WINGCALC
      INTEGER WINGMOD,EGGMULT,ARRHEN,STARVEMODE,FOODMODE,COND_CLUTCH
...
C     CHECK IF FIRST DAY OF SIMULATION
      ...
      IF(RESET.EQ.DBLE(STAGES+1))THEN
...
C     IF NOT PUPATED YET, STRUCTURE IS LARVAL STRUCTURE 'VOLD'
      IF(STAGE.LT.DBLE(STAGES-2))THEN
       V_PRES=VOLD_PRES
      ENDIF

C     ACCELERATION CHECK
      IF(STAGE.GE.DBLE(STAGES-2))THEN
...
C     FINAL CRITICAL WEIGHT THRESHOLD
     ...
     &((VDOTREF/L_B)-G*K_M)))
      ...
      IF((STAGE.GT.0.).AND.(STAGE.LT.DBLE(STAGES-3)))THEN
C      LARVA, USE CRITICAL LENGTH THRESHOLDS
       L_THRESH=L_INSTAR(INT(STAGE))
      ENDIF
      IF(STAGE.EQ.DBLE(STAGES-3))THEN
...
c     FOR SIZE-DEPENDENT CLUTCH (SET CLUTCH SIZE BASED ON SIZE WHEN P_B IS NOT INCREASING BEFORE EGG ALLOCATION COMMENCES)
      IF((CLUTCHA.GT.0).AND.(P_B.LE.0.0))THEN
...
1       CONTINUE
      ...
      IF((STAGE.GT.0.).AND.(STAGE.NE.DBLE(STAGES-2)))THEN
     ...
     &  *((VTDOT/L_PRES)-R)-R*Q_PRES
      ...
      IF(STAGE.GE.DBLE(STAGES-1))THEN
       IF(HOUR.EQ.1)THEN
        Q(HOUR)=Q_INIT + DQDT
       ELSE
...
C     TRANSITION BETWEEN STAGES FOR INSECT
      IF(STAGE.EQ.0.)THEN
       ...
       ENDIF
      ELSE
       IF(STAGE.LT.DBLE(STAGES-3))THEN
...
C     & *ETA_PA))/3600./TCORR   
      ...
      IF((STAGE.GT.0.).OR.(STAGE.NE.DBLE(STAGES-2)))THEN ! ASSUME PUPAL STAGE CAN'T FEED
     ...
     &+((CUMBATCH(HOUR)/MU_E)*W_E)/EGGDRYFRAC
      IF(STAGE.GT.DBLE(STAGES-2))THEN
...
C      GOTO 987
      IF(STAGE.NE.DBLE(STAGES-2))THEN
