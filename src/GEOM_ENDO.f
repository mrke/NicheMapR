...
C     EFFECTIVE AREA OF SKIN THAT IS WET).
     ...
     & ,SHAPEB,SHAPEB_REF,SHAPEC,DHARA,RHOARA,PCOND,SAMODE,ORIENT
     & ,RESULTS)

      IMPLICIT NONE
      
      DOUBLE PRECISION A2,AHEIT,ALENTH,AMASS,ANDENS,AREA,AREASKIN,ASEMAJ
      DOUBLE PRECISION ASEMAJREF,ASIL,ASILN,ASILP,ATOT,AV,AWIDTH
      DOUBLE PRECISION BSEMIN,BSEMINREF,C2,CFA,CFB,CFC,CFD,CFT1,CFT1A
      DOUBLE PRECISION CFT1B,CFT1C,CFT2,CFT2A,CFT2B,CFT3,CFT4,CFT5
      DOUBLE PRECISION CONVAR,CONVSK,CSEMIN,CSEMINREF,D,DHAIR,DHARA,E,E2
      DOUBLE PRECISION FATDEN,FATPCT,FATTHK,FLSHASEMAJ,FLSHBSEMIN
      DOUBLE PRECISION FLSHCSEMIN,FLSHVL,GMASS,HAIRAR,MASFAT,ORIENT
      DOUBLE PRECISION PCOND,PI,R1,R2,RESULTS,RFLESH,RHO,RHOARA,SAMODE
      DOUBLE PRECISION SHAPE,SHAPEB,SHAPEB_REF,SHAPEC,SUBQFAT,VOL,VOLFAT
      DOUBLE PRECISION ZFUR
...
C       SO B = (((3/(4*G))*VOL)/PI)^(1/3)
        ...
        BSEMINREF=(((3./(4.*SHAPEB_REF))*VOL)/PI)**(1./3.) ! SAVE REFERENCE VALUE BSEMINREF FOR USE IN ASILN CALCULATIONS.
        ASEMAJREF = SHAPEB_REF*BSEMINREF ! SAVE REFERENCE VALUE ASEMAJREF FOR USE IN ASILN CALCULATIONS.
        CSEMIN=BSEMIN
        CSEMINREF=BSEMIN
        ...
        ASILN = PI * (ASEMAJREF + ZFUR) * (BSEMINREF + ZFUR)
        ASILP = PI * (BSEMINREF + ZFUR) * (CSEMINREF + ZFUR)
        IF(ORIENT.EQ.0)THEN ! AVERAGE POSTURE
         ASIL = (ASILN + ASILP)/2.
        ENDIF
        IF(ORIENT.EQ.1)THEN ! NORMAL TO SUN'S RAYS (HEAT MAXIMISING)
...
C       SKIN SURFACE AREA
        ...
        ELSE
         ...
         ENDIF
        ENDIF
C       R1 = BSEMINREF ! FOLLOWING DISCUSSIONS WITH MIKE MAKE THIS BASED ON BSEMIN. MAY NOT WORK WELL IF SHAPEB IS LARGE.
        R1 = BSEMIN
        R2 = R1 + ZFUR
        A2 = ASEMAJ + ZFUR
        C2 = R1 + ZFUR
...
C       AREA AT THE FUR TIPS
     ...
     &      /10000. ! INCREASE IN AREA DUE TO FUR
        ...
        ENDIF
        ATOT = AREA
C       D = (BSEMINREF+ZFUR)*2 !MINOR AXIS PERPENDICULAR TO FLOW  
        D = VOL**(1./3.)
        FLSHVL = VOL
        RFLESH = R1
        FATTHK = 0.    
...
C       AREA AT THE FUR TIPS
     ...
     &      /10000. ! INCREASE IN AREA DUE TO FUR
        ...
        ENDIF
        ATOT = AREA
C       D = (BSEMINREF+ZFUR)*2 !MAJOR AXIS PERPENDICULAR TO FLOW
        D = VOL**(1./3.)
       ENDIF
       GO TO 999
      ENDIF
