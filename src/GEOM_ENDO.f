...
C       WHICH TAKES THE FORM: AX^3 + BX^2 + CX + D = 0.
C       SOLVE FOR FAT THICKNESS FATTHK USING NEWTON'S METHOD.
C       F(X)=(FLSHASEMAJ+X)*(FLSHBSEMIN+X)**2=(3/(4*PI))*(VOLFAT+FLSHVL)
C       F IS STRICTLY MONOTONE FOR X.GT.-FLSHBSEMIN, SO THERE IS EXACTLY
C       ONE NON-NEGATIVE ROOT. 10 FIXED ITERATIONS REPLACES THE CARDANO
C       FORMULA, WHICH RETURNS THE WRONG ROOT WHEN THE DISCRIMINANT IS
C       NEGATIVE (CASUS IRREDUCIBILIS).
        CFT5=(3.0D0/(4.0D0*PI))*(VOLFAT+FLSHVL)
        FATTHK=0.0D0
        DO 15 ITER=1,10
         CFT1=FLSHBSEMIN+FATTHK
         CFT2=FLSHASEMAJ+FATTHK
         CFT3=CFT2*CFT1**2.-CFT5
         CFT4=CFT1**2.+2.*CFT2*CFT1
         FATTHK=FATTHK-CFT3/CFT4
15      CONTINUE
