...
C     BREEDING PHENOLOGY FOR DEB MODEL
      ...
      DOUBLE PRECISION LAMBDA,LAT,PI,MLENGTH,FLENGTH,LENGTHDAY
      DOUBLE PRECISION DAYLENGTHSTART,DAYLENGTHFINISH
      DOUBLE PRECISION PREVDAYLENGTH,LENGTHDAYDIR

      INTEGER PHOTOSTART,PHOTOFINISH,DAYCOUNT,IHOUR,STARTDAY,BREEDSTART
      INTEGER BREEDEND,I,BREED,PHOTODIRS,PHOTODIRF,DOY
...
C     SETTING STARTDAY FOR PHOTOPERIOD-SPECIFIED BREEDING PERIODS WHEN STARTDAY SET TO ZERO BY USER
     ...
     &  (STARTDAY.EQ.0))THEN
       DO 844 I=1,365
        MLENGTH=1-TAN(LAT*PI/180)*TAN(23.439*PI/180*COS(0.0172*(I-1)))
...
C     SAVING LAST DAY'S LENGTH FOR CHECK FOR INCREASING OR DECREASING DAY LENGTH
      ...
      ELSE
       IF(DOY.EQ.1)THEN
        MLENGTH=1-TAN(LAT*PI/180)*TAN(23.439*PI/180*COS(0.0172*(365-1)))
        IF(MLENGTH.GT.2)THEN
         MLENGTH=2
        ENDIF
        IF(MLENGTH.LT.0)THEN
...
C       NOW CHECK FOR WHEN BREEDING SEASON WOULD START
     ...
     &(LENGTHDAY.LE.DAYLENGTHSTART).AND.(FLENGTH*24.GT.LENGTHDAY).AND.
     &(PHOTODIRS.EQ.1))THEN
          BREEDSTART=I
         ENDIF
         IF((FLENGTH*24.LT.DAYLENGTHSTART).AND.(BREED.EQ.0).AND.
     &(LENGTHDAY.GE.DAYLENGTHSTART).AND.(FLENGTH*24.LT.LENGTHDAY).AND.
     &(PHOTODIRS.EQ.0))THEN
          BREEDSTART=I
         ENDIF
         LENGTHDAY=FLENGTH*24
...
C       NOW CHECK FOR WHEN BREEDING SEASON WOULD END
     ...
     &(LENGTHDAY.LE.DAYLENGTHFINISH).AND.(FLENGTH*24.GT.LENGTHDAY).AND.
     &(PHOTODIRF.EQ.1))THEN
          BREEDEND=I
         ENDIF
         IF((FLENGTH*24.LT.DAYLENGTHFINISH).AND.(BREED.EQ.1).AND.
     &(LENGTHDAY.GE.DAYLENGTHFINISH).AND.(FLENGTH*24.LT.LENGTHDAY).AND.
     &(PHOTODIRF.EQ.0))THEN
          BREEDEND=I
         ENDIF
         LENGTHDAY=FLENGTH*24
...
C        NORTHERN HEMISPHERE
        ...
        IF((BREEDSTART.GT.0).AND.(BREEDEND.GT.0))THEN
         IF(BREEDEND-BREEDSTART.LE.0)THEN
          LAMBDA=(365-BREEDSTART+BREEDEND)/365.
         ELSE
          LAMBDA=(BREEDEND-BREEDSTART)/365.
         ENDIF
        ENDIF
       ENDIF
