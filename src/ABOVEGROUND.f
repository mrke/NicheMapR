...
C     COMPUTES ABOVEGROUND MICROCLIMATE TO WHICH THE ANIMAL IS EXPOSED
      ...
      DOUBLE PRECISION K_SAT,BB,KSATS,BBS
      ...
      DIMENSION ZSOIL(10),TCOND(25),SHTCOND(25),KSATS(10),BBS(10)
...
C	  SOIL ENVIRONMENT FOR CONDUCTION AND LIQUID WATER EXCHANGE
      SUBTK=TCOND(IHOUR)*((MAXSHD-SHADE)/MAXSHD)+(SHTCOND(IHOUR) ! WEIGHTED MEAN OF VALUE IN MIN AND MAX AVAILABLE SHADE ACCORDING TO %SHADE CHOSEN
     & *(SHADE/MAXSHD))
      POT=PSOIL(IHOUR)*((MAXSHD-SHADE)/MAXSHD)+(PSHSOI(IHOUR) ! WEIGHTED MEAN OF VALUE IN MIN AND MAX AVAILABLE SHADE ACCORDING TO %SHADE CHOSEN
     & *(SHADE/MAXSHD)) 
      BB=BBS(1)
      K_SAT=KSATS(1)
