...
C     CONVECTIVE AND HUMIDITY ENVIRONMENT
      IF(FLYTEST.EQ.1)THEN ! ASSUME FLYING AT REFERENCE HEIGHT
       IF(FLYHIGH.EQ.1)THEN
        TA = TREF(IHOUR)
        RELHUM = RHREF(IHOUR)
        VEL = MAX(FLYSPEED, VREF(IHOUR))
       ELSE
     ...
     &   (TSHLOW(IHOUR)*(SHADE/MAXSHD))
        RELHUM = RH(IHOUR)
        VEL = MAX(FLYSPEED, VLOC(IHOUR))
       ENDIF
      ELSE
       IF(CLIMBING.EQ.1)THEN ! ASSUME HAS CLIMBED TO REFERENCE HEIGHT
        TA = TREF(IHOUR)
        VEL = VREF(IHOUR)
        RELHUM = RHREF(IHOUR)
       ELSE
