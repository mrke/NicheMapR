...
C     IF METAB_MODE=1. USES EULER INTEGRATION
      ...
      DOUBLE PRECISION H_A,H_AREF,HALFSAT,HS,HS_INIT,HS_PRES,HRN,JM_JO
      DOUBLE PRECISION JMCO2,JMCO2_GM,JMH2O,JMH2O_GM,JMNWASTE
      DOUBLE PRECISION JMNWASTE_GM,JMO2,JMO2_GM,JOJE,JOJE_GM,JOJP
      DOUBLE PRECISION JOJP_GM,JOJV,JOJV_GM,JOJX,JOJX_GM,K_J,K_JREF,K_M
      DOUBLE PRECISION KAP,KAP_G,KAP_R,KAP_X,KAP_X_P,KAPPA,L_B,L_J,L_M
      DOUBLE PRECISION L_PRES,L_T,L_THRESH,L_W,L_WREPRO,LAMBDA,LAT
      DOUBLE PRECISION LENGTHDAY,LENGTHDAYDIR,LONGEV,M_V,MAXMASS
      DOUBLE PRECISION MINCLUTCH,MINED,MLO2,MLO2_INIT,MONMATURE,MONREPRO
      DOUBLE PRECISION MR_1,MR_2,MR_3,MU_AX,MU_E,MU_P
...
C      USE EMBRYO EQUATION FOR LENGTH, FROM KOOIJMAN 2009 EQ. 2
      ...
      ELSE
       IF((METAB_MODE.EQ.1).AND.(E_H_PRES.GE.E_HJ))THEN
        ...
        P_C=E_PRES*V_PRES*VDOT/L_PRES! # J / t, mobilisation rate (not that v is already corrected for s_M
        P_M=KAP*P_C ! new absolute flow to maintenance
       ELSE
        P_C=E_PRES*(VDOT/L_PRES-R)*V_PRES ! J / t, mobilisation rate, equation 2.12 DEB3
       ENDIF
       DVDT = V_PRES*R
...
898   CONTINUE
      ...
      IF(E_H(HOUR).GT.E_HB)THEN
       IF(ACTHR(HOUR).GT. 1)THEN
        DESDT = P_XM*((X_FOOD/HALFSAT)/(1+X_FOOD/HALFSAT))*V(HOUR)**
     &   (2./3.)*FUNCT-1.*(P_AM/KAP_X)*V(HOUR)**(2./3.)
       ELSE
        DESDT = -1.*(P_AM/KAP_X)*V(HOUR)**(2./3.)
       ENDIF
      ELSE
       DESDT = -1.*(P_AM/KAP_X)*V(HOUR)**(2./3.)
      ENDIF
