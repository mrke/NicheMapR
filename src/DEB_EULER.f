...
C     IF METAB_MODE=1. USES EULER INTEGRATION
      ...
      DOUBLE PRECISION E_PRES,E_SCALED,E_TEMP,ECTOINPUT,ED,EGGDRYFRAC
      ...
      DOUBLE PRECISION GH2OMET_INIT,GUTFILL
      ...
      DOUBLE PRECISION M_V,MAXMASS,MAXCLUTCH,MINCLUTCH,MINED,MLO2
      DOUBLE PRECISION MLO2_INIT,MONMATURE,MONREPRO,MR_1,MR_2,MR_3,MU_AX
      DOUBLE PRECISION MU_E,MU_M,MU_N,MU_O,MU_P,MU_V,MU_X,NEWCLUTCH
      DOUBLE PRECISION NWASTE,O2FLUX,ORIG_CLUTCHSIZE,ORIG_E_SM,P_A,P_AM
      DOUBLE PRECISION P_B,P_C,P_D,P_G,P_J,P_M,P_MREF,P_MV,P_R,P_X,P_XM
      DOUBLE PRECISION P_XMREF,PAS,PBS,PCS,PDS,PGS,PJS,PMS,PRS,PFEWAT
      DOUBLE PRECISION PHI,PHIMAX,PHIMIN,PI,POND_DEPTH,POTFREEMASS
      DOUBLE PRECISION PREVDAYLENGTH,PTCOND,PTCOND_ORIG,Q,Q_INIT,Q_PRES
      DOUBLE PRECISION QCOND,QCONV,QIRIN,QIROUT,QMETAB,QRESP,QSEVAP,QSOL
      DOUBLE PRECISION QSOLAR,R,RAINDRINK,RAINFALL,RELHUM,REPRO,RESID,RH
      DOUBLE PRECISION RHO1_3,RHREF,RQ,S_G,S_J,S_M,SC,SCALED_L,SIDEX,SIG
      DOUBLE PRECISION STAGE,STAGE_REC,STARVE,SUBTK,SURVIV,SURVIV_INIT
      DOUBLE PRECISION SURVIV_PRES,T_A,T_A2,T_AH,T_AH2,T_AL
      DOUBLE PRECISION T_AL2,T_H,T_H2,T_L,T_L2,T_REF,TA,TALOC,MR_4
      DOUBLE PRECISION TB,TBASK,TC,TCORES,TCORR,TCORR2,TDIGPR,TEMERGE
      DOUBLE PRECISION TIME,TLUNG,TMAXPR,TMINPR,TPREF,TQSOL
      ...
      DOUBLE PRECISION YEX,YPX,YXE,ZFACT,CUMWATER,REMH2O,LIQPAST
      ...
      INTEGER CTKILL,CTMINCUM,CTMINTHRESH,COND_CLUTCH,DAYCOUNT,DEAD
      INTEGER DEADEAD,DEB1,DEHYDRATED,DOY,F1COUNT,FEEDING,FIRSTDAY,HOUR
      INTEGER I,INWATER,IYEAR,METAB_MODE,METAMORPH,NN,NYEAR,PHOTODIRF
      ...
      DIMENSION E_BABY1(24),E_H(24),E_HPUP(24),ECTOINPUT(169)
...
C       USE EMBRYO EQUATION FOR SCALED RESERVE, U_E, FROM KOOIJMAN 2009 EQ. 1
     ...
     &   VDOT))
        ...
        DEDT=E_TEMP-E_PRES
       ELSE
        IF(E_S_INIT*KAP_X.GT.P_A)THEN
C        EQUATION 2.10 DEB3
         DEDT = P_A/V_PRES-(E_PRES*VDOT)/L_PRES
        ELSE
         DEDT = E_S_INIT*KAP_X/V_PRES-(E_PRES*VDOT)/L_PRES
        ENDIF
       ENDIF
      ELSE
       IF(E_H_PRES.LT.E_HB)THEN
C       USE EMBRYO EQUATION FOR SCALED RESERVE, U_E, FROM KOOIJMAN 2009 EQ. 1
     ...
     &   VDOT))
        ...
        DEDT=E_TEMP-E_PRES
       ELSE
        IF(E_S(HOUR-1)*KAP_X.GT.P_A)THEN
         DEDT = P_A/V_PRES-(E_PRES*VDOT)/L_PRES
        ELSE
         DEDT = MAX(0.0D0,E_S(HOUR-1)*KAP_X/V_PRES)-(E_PRES*VDOT)/L_PRES
        ENDIF
       ENDIF
      ENDIF
...
C     END CHECK FOR IMMATURE OR MATURE
      ...
      IF(STARVEMODE.GT.0)THEN
       ...
       IF(HOUR.EQ.1)THEN
        IF((STARVE.GT.0.).AND.(CUMREPRO_INIT.GT.STARVE))THEN
         P_R = P_R - STARVE
         IF(E_H(HOUR).LT.E_HP)THEN ! TAKE BACK SOME OF THE MATURITY PREVIOUSLY DETERMINED
          E_H(HOUR) = E_H(HOUR) - STARVE
         ENDIF         
         STARVE = 0.
         STARVING = 0
        ENDIF
     ...
     &   STARVE))THEN
      ...
      IF(STARVEMODE.EQ.2)THEN
       ...
       IF((E_H(HOUR).GE.E_HP).AND.((ED(HOUR)/E_M).LT.0.95))THEN
        IF(DEDT.LT.0)THEN
         IF(CUMREPRO(HOUR).GT.(-1.*DEDT*V_PRES))THEN
          p_R = p_R + DEDT * V_PRES
          ED(HOUR) = E_PRES
         ENDIF
         IF(CUMBATCH(HOUR).GT.(-1.*DEDT*V_PRES))THEN
          p_B = p_B + DEDT * V_PRES
          ED(HOUR) = E_PRES
         ENDIF 
        ELSE
...
c     FOR SIZE-DEPENDENT CLUTCH (SET CLUTCH SIZE BASED ON SIZE WHEN P_B IS NOT INCREASING BEFORE EGG ALLOCATION COMMENCES)
