...
C      THIS SUBROUTINE COMPUTES METABOLIC HEAT
     ...
     & PAS,PBS,PCS,PDS,PGS,PJS,PMS,PRS,WETFOOD 
      DOUBLE PRECISION STAGE,RAINDRINK,POT,MR_4
      ...
      INTEGER CENSUS,RESET,DEADEAD,STARTDAY,DEAD,EGGMULT,ARRHEN,FLYHIGH

      CHARACTER BURROW,CLIMB,CKGRSHAD,CREPUS,DAYACT,NOCTURN
...
C       CAP METABOLIC RATE EQUATION WITH MAX OF TC = 50
        QMETAB=(20.1/3600.0)*10.**(MR_3*50.)*MR_1*GMASS**MR_2*
     &   10.**MR_4+FLYMETAB
        GOTO 101
       ENDIF
       !IF(TC.LE.TPREF)THEN
...
C        ACCEPTABLE TEMPERATURE RANGE
         QMETAB=(20.1/3600.0)*10.**(MR_3*TC)*MR_1*GMASS**MR_2*
     &    10.**MR_4+FLYMETAB
        ELSE
         QMETAB=(20.1/3600.0)*10.**(MR_3*1.0)*MR_1*GMASS**MR_2*
     &    10.**MR_4+FLYMETAB
        ENDIF
       !ELSE
C       TOO HOT
...
C     IF(FLYTEST.EQ.1)THEN
C      QMETAB=QMETAB+FLYMETAB
C     ENDIF
