...
C     IF METAB_MODE=1. USES DOPRI5 ALGORITHM FOR INTEGRATION
      ...
      DOUBLE PRECISION MU_AX,MU_E,MU_M,MU_N,MU_O,MU_P,MU_V,MU_X
      DOUBLE PRECISION NEWCLUTCH,NWASTE,O2FLUX,ORIG_CLUTCHSIZE,ORIG_E_SM
      DOUBLE PRECISION P_A,P_AM,P_B,P_C,P_D,P_G,P_J,P_M,P_MREF,P_MV,P_R
      DOUBLE PRECISION P_X,P_XM,P_XMREF,PAS,PBS,PCS,PDS,PGS,PJS,PMS,PRS
      DOUBLE PRECISION PFEWAT,PHI,PHIMAX,PHIMIN,PI,POND_DEPTH
      ...
      DOUBLE PRECISION CUMWATER,LIQPAST
      ...
      INTEGER RAINHOUR,EGGMULT,ARRHEN,STARVEMODE,FOODMODE
...
C     FOR SIZE-DEPENDENT CLUTCH
      IF((CLUTCHA.GT.0).AND.(E_HB.NE.E_HO))THEN ! MAKE SURE CLUTCH SIZE DOESN'T INCREASE DURING PREGNANCY BECAUSE OTHERWISE GET JUMP UP IN MASS OF GONAD
       CLUTCHSIZE=FLOOR(CLUTCHA*(V_PRES**(1./3.)/DELTA_DEB)-CLUTCHB)
       NEWCLUTCH=CLUTCHSIZE
      ENDIF
C     IF(CLUTCHSIZE.GT.ORIG_CLUTCHSIZE)THEN
C      CLUTCHSIZE=ORIG_CLUTCHSIZE
C      NEWCLUTCH=CLUTCHSIZE
C     ENDIF
