...
C     IF METAB_MODE=1. USES DOPRI5 ALGORITHM FOR INTEGRATION
      ...
      DOUBLE PRECISION DEPRESS,DEPSEL,DEPSUB,DRYFOOD,DSURVDT,E_BABY
      DOUBLE PRECISION E_BABY_INIT,E_BABY1,E_EGG,E_G,E_H,E_H_INIT
      DOUBLE PRECISION E_H_PRES,E_H_START,E_HB,E_HE,E_HJ,E_HO,E_HP
      DOUBLE PRECISION E_HPUP,E_HPUP_INIT,E_INIT,E_INIT_BABY,E_M,E_M2
      DOUBLE PRECISION E_PRES,E_SCALED,ECTOINPUT,ED,EGGDRYFRAC,EGGSOIL
      DOUBLE PRECISION EH_BABY,EH_BABY_INIT,EH_BABY1,EMISAN,EPUP
      DOUBLE PRECISION EPUP_INIT,E_S,E_S_INIT,E_S_PAST,E_S_PRES,E_SM
      DOUBLE PRECISION ETA_PA,ETAO,EXTREF,F,F12,F13,F14,F15,F16,F21,F23
      DOUBLE PRECISION F24,F25,F26,F31,F32,F41,F42,F51,F52,F61,FAECES
      DOUBLE PRECISION FATOSB,FATOSK,FECUNDITY,FLSHCOND,FOOD
      DOUBLE PRECISION FOODLEVELCUR,FOODLIM,FOODWATERCUR,FUNCT,G,GH2OMET
      ...
      DOUBLE PRECISION JMO2_GM,J_M,J_O,J_P,JOJE,JOJE_GM,JOJP,JOJP_GM
      DOUBLE PRECISION JOJV,JOJV_GM,JOJX,JOJX_GM,K_EL,K_EV,K_J,K_JREF
      DOUBLE PRECISION K_M,KAP,KAP_V,KAP_R,KAP_X,KAP_G,KAP_X_P,L_B,L_J
      DOUBLE PRECISION L_M,L_PRES,L_T,L_THRESH,LAMBDA,LAT,LENGTHDAY
      DOUBLE PRECISION LENGTHDAYDIR,LONGEV,M_V,MAXCLUTCH,MAXMASS
      DOUBLE PRECISION MINCLUTCH,MINED,MLO2,MLO2_INIT,MONMATURE,MONREPRO
      DOUBLE PRECISION MR_1,MR_2,MR_3,MU_AX,MU_E,MU_M,MU_N,MU_O,MU_P
      DOUBLE PRECISION MU_V,MU_X,NEWCLUTCH,NWASTE,O2FLUX,ORIG_CLUTCHSIZE
      DOUBLE PRECISION ORIG_E_SM,P_A,P_AM,P_B,P_C,P_D,P_G,P_J,P_M,P_MREF
      DOUBLE PRECISION P_MV,P_R,P_X,P_XM,P_XMREF,PAS,PBS,PCS,PDS,PGS,PJS
      DOUBLE PRECISION PMS,PRS,PFEWAT,PHI,PHIMAX,PHIMIN,PI,POND_DEPTH
      DOUBLE PRECISION POTFREEMASS,PREVDAYLENGTH,PTCOND,PTCOND_ORIG,Q
      DOUBLE PRECISION Q_INIT,Q_PRES,QCOND,QCONV,QIRIN,QIROUT,QMETAB
      DOUBLE PRECISION QRESP,QSEVAP,QSOL,QSOLAR,R,RAINDRINK,RAINFALL
      DOUBLE PRECISION RAINMULT,RELHUM,REMH2O,REPRO,RESID,RH
      DOUBLE PRECISION RHO1_3,RHREF,RQ,S_M,S_G,S_J,SC,SCALED_L,SIDEX,SIG
      DOUBLE PRECISION STAGE,STAGE_REC,STARVE,SUBTK,SURVIV,SURVIV_INIT
      DOUBLE PRECISION SURVIV_PRES,L_W,L_WREPRO,T_A,T_A2,T_AH,T_AH2,T_AL
      DOUBLE PRECISION T_AL2,T_H,T_H2,T_L,T_L2,T_REF
      DOUBLE PRECISION TA,TALOC,TB,TBASK,TC,TCORES,TCORR,TCORR2,TDIGPR
      DOUBLE PRECISION TEMERGE,TIME,TLUNG,TMAXPR,TMINPR,TPREF
      ...
      DOUBLE PRECISION WETMASS,WETSTORAGE,WQSOL,X_FOOD,YEX,YPX
      DOUBLE PRECISION YXE,ZFACT,WORK,RPAR,RTOL,ATOL,YY,XEND,X
      DOUBLE PRECISION EGGPTCOND,POT,H_O,H_M,CNWASTE,CH2OMET
      DOUBLE PRECISION CUMWATER,LIQPAST,MR_4
      ...
      INTEGER RAINHOUR,EGGMULT,ARRHEN,STARVEMODE,FOODMODE,COND_CLUTCH
      
      CHARACTER TRANST

      DIMENSION ACTHR(25),BREEDVECT(24),CUMBATCH(24),CUMREPRO(24)
      DIMENSION DEBFIRST(15),DEBQMET(24),DEPSEL(25),DRYFOOD(24)
      DIMENSION E_BABY1(24),E_H(24),E_HPUP(24),ECTOINPUT(169),ED(24)
      DIMENSION EGGSOIL(24),EH_BABY1(24),EPUP(24),E_S(24),ETAO(4,3)
      DIMENSION FAECES(24),FOOD(50),GH2OMET(24),HS(24),HRN(25)
      DIMENSION J_O(4),J_M(4),JM_JO(4,4),MLO2(24),MU_O(4),MU_M(4)
      DIMENSION NWASTE(24),PAS(24),PCS(24),PMS(24)
      DIMENSION PGS(24),PDS(24),PJS(24),PRS(24),PBS(24),Q(24),REPRO(24)
      DIMENSION RHREF(25),STAGE_REC(25),SURVIV(24),L_W(24),TALOC(25)
      ...
      DIMENSION IPAR(31),RPAR(35),VSTI(10)
...
C     METABOLIC ACCELERATION IF PRESENT
      ...
      X_FOOD = FOODLEVELCUR!S(DAYCOUNT)
...
C     ENDIF
      ...
      X_FOOD = FOODLEVELCUR!FOODLEVELS(DAYCOUNT)
...
C     TEMPERATURE CORRECTIONS AND COMPOUND PARAMETERS
      ...
      IF(((AESTIVATE.EQ.1).AND.(AQUATIC.EQ.1).AND.(POND_DEPTH.LE.0.))
     & .OR.((AESTIVATE.EQ.1).AND.(DEHYDRATED.EQ.1)).OR.
     & ((AESTIVATE.EQ.1).AND.(E_SCALED.LT.0.8).AND.(X_FOOD.LE.0.)))THEN
       AEST=1
       E_S_INIT=0.
      ELSE
       AEST=0
...
C     DIAPAUSE BEFORE POND FILL
      IF(AQUABREED.EQ.1)THEN
       IF((E_H_PRES.GE.E_HB).AND.(STAGE.EQ.0.))THEN
...
C --- INITIAL VALUES AND ENDPOINT OF INTEGRATION
      ...
      RPAR(30)=DBLE(F)
      RPAR(31)=DBLE(METAB_MODE)
      RPAR(32)=DBLE(E_HJ)
      RPAR(33)=DBLE(STARVEMODE)
      RPAR(34)=DBLE(FOODMODE)
      RPAR(35)=DBLE(E_SM)
      X=0.0D0
      YY(1)=0.0D0
      YY(2)=MAX(DBLE(V_PRES),0.0D0)
      YY(3)=MAX(DBLE(E_PRES),0.0D0)
      YY(4)=MAX(DBLE(E_H_PRES),0.0D0)
      YY(5)=MAX(DBLE(E_S_PRES),0.0D0)
      YY(6)=MAX(DBLE(STARVE),0.0D0)
      YY(7)=MAX(DBLE(Q_PRES),0.0D0)
      YY(8)=MAX(DBLE(HS_PRES),0.0D0)
      YY(9)=MAX(DBLE(CUMREPRO_INIT),0.0D0)
      YY(10)=MAX(DBLE(CUMBATCH_INIT),0.0D0)
      XEND=1.D+00
...
C --- CALL OF THE SUBROUTINE DOPRI5
     ...
     & WORK,LWORK,IWORK,LIWORK,RPAR,IPAR,IDID,VSTI)
      V(HOUR)=MAX(YY(2),0.0D0)
      ED(HOUR)=MAX(YY(3),0.0D0)
      E_H(HOUR)=MAX(YY(4),0.0D0)
      E_S(HOUR)=MAX(YY(5),0.0D0)
      IF(E_S(HOUR).GT.(E_SM*V(HOUR)))THEN
       RESID=E_S(HOUR)-E_SM*V(HOUR) !# excess food intake to stomach capacity
      ...
      E_S_INIT=E_S(HOUR)
      E_S_PAST=E_S(HOUR)
      STARVE=MAX(YY(6),0.0D0)
      Q(HOUR)=MAX(YY(7),0.0D0)
      HS(HOUR)=MAX(YY(8),0.0D0)
      P_R=MAX(YY(9)-CUMREPRO_INIT,0.0D0)
      P_B=MAX(YY(10)-CUMBATCH_INIT,0.0D0)
      IF(E_H(HOUR).GE.E_HP)THEN
       CUMREPRO(HOUR)=MAX(YY(9),0.0D0)
       CUMBATCH(HOUR)=MAX(YY(10),0.0D0)
      ELSE
       CUMREPRO(HOUR)=CUMREPRO_INIT
       CUMBATCH(HOUR)=CUMBATCH_INIT
      ENDIF
      E_SCALED=ED(HOUR)/E_M ! REDEFINE E_SCALED TO NEW ED(HOUR)
      L_W(HOUR) = V(HOUR)**(1./3.)/DELTA_DEB*10.0D0 ! LENGTH IN MM
...
c     FOR SIZE-DEPENDENT CLUTCH
      ...
      IF((CLUTCHA.GT.0).AND.(EH_BABY.LE.0.0))THEN
       CLUTCHSIZE=MAX(FLOOR((CLUTCHA*(V_PRES**(1./3.)/DELTA_DEB)
     &  +CLUTCHB)),INT(MINCLUTCH))
       IF(MAXCLUTCH.GT.0.)THEN
        CLUTCHSIZE=MIN(MAXCLUTCH,CLUTCHSIZE) ! CAP AT MAX CLUTCH SIZE
       ENDIF
       NEWCLUTCH=CLUTCHSIZE
      ENDIF
      IF(COND_CLUTCH.EQ.1)THEN
        CLUTCHSIZE=E_SCALED*CLUTCHSIZE
      ENDIF
      IF(CLUTCHSIZE.LT.1)THEN
       CLUTCHSIZE=MAX(1,INT(MINCLUTCH))
       NEWCLUTCH=CLUTCHSIZE
      ENDIF
...
C     POWERS
      P_M = P_MV*V(HOUR)
      P_J = MAX(0.0D0,K_J*E_H(HOUR) - STARVE)
      IF(E_S(HOUR).GT.V(HOUR)**(2./3.)*P_AM*F)THEN
...
C     ABJ MODEL
      ...
      IF(METAB_MODE.EQ.1)THEN
       IF((STAGE.GT.0.).AND.(STAGE.LT.DBLE(STAGES-1)))THEN
C       LARVA/NYMPH, USE CRITICAL LENGTH THRESHOLDS
       ...
       ENDIF
      ENDIF
      IF(STAGE.GT.DBLE(STAGES-1))THEN
       STAGE=DBLE(STAGES-1)
      ENDIF
...
C     REPRODUCTION
      ...
      IF(CUMBATCH(HOUR).GT.0)THEN
       IF(MONMATURE.EQ.0)THEN
        MONMATURE=DBLE(COUNTDAY+365*(IYEAR-1))/30.5
       ENDIF
      ENDIF
       ...
       IF(VIVIPAROUS.EQ.1)THEN
        ...
        IF(EH_BABY.GE.E_HO)THEN
         ...
         IF(FECUNDITY.GE.CLUTCHSIZE)THEN
          MONREPRO=DBLE(COUNTDAY+365.*(IYEAR-1))/30.5
          L_WREPRO=L_W(HOUR)
         ENDIF
         PREGNANT=0
        ENDIF
...
C     J FOOD EATEN PER HOUR
      IF(E_H(HOUR).GE.E_HB)THEN
       IF(ACTHR(HOUR).GT.1)THEN
        P_X = F * P_XM * V(HOUR)**(2./3.)*((X_FOOD / HALFSAT)
...
C      SEND THE ALLOMETRIC VALUE TO THE OUTPUT FILE
       O2FLUX = 10.**(MR_3*TC)*MR_1*(AMASS*1000.)**MR_2*10.**MR_4 ! REGRESSION-BASED
      ENDIF
      MLO2(HOUR)=O2FLUX
...
C     & *ETA_PA))/3600./TCORR
     ...
     & +((CUMBATCH(HOUR)/MU_E)*W_E)/EGGDRYFRAC
      ...
      ENDIF
      WETFOOD(HOUR) = ((E_S(HOUR)/MU_E)*W_E)/(1.-FOODWATERS(HOUR)) ! WET FOOD MASS (G)
      IF(STAGE.EQ.0.)THEN
...
C     STATE OF BABY IF VIVIPAROUS AND PREGNANT     
      ...
      IF(CONTH.EQ.0)THEN
       IF((VIVIPAROUS.EQ.1).AND.(E_H(HOUR).LT.E_HO))THEN
...
C     RESET IF DEAD
      ...
      IF((DEAD.EQ.0).AND.(V(HOUR).GT.V_PRES))THEN
       DEAD=0
      ENDIF
