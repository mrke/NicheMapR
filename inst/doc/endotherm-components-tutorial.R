...
# call the subroutine
GEOM.out <- GEOM_ENDO(AMASS, ANDENS, FATDEN, FATPCT, SHAPE, ZFUR, SUBQFAT, SHAPE_B, SHAPE_C, DHARA, RHOARA, PCOND, SAMODE, ORIENT, ZEN)
...
AREACOND <- ACOND # conduction area, m2
...
SOLAR.out <- SOLAR_ENDO(AREA, AREACOND, ABSAND, ABSANV, ABSSB, ASIL, PDIF, QNORM, SHADE, QSOLR, FASKY, FAVEG)
...
# run subroutine
CONV.out <- CONV_ENDO(TS, TENV, SHAPE, SURFAR, FLTYPE, FURTST, D, TFA, VEL, ZFUR, BP, ELEV, GRAV, O2GAS, N2GAS, CO2GAS, CONV_ENHANCE)
...
# Setting config factors and solar depending on whether the dorsal side (S=1) or ventral side (S=2) is being estimated.
  ...
  # package up inputs
  FURVARS <- c(LEN,ZFUR,FURTHRMK,KEFF,BETARA[S+1],FURTST,ZL,LHAR[S+1],DHAR[S+1],RHOAR[S+1],REFLFR[S+1],KHAIR,S)
  ...
  # call SIMULSOL
  SIMULSOL.out[S,] <- SIMULSOL(DIFTOL, IPT, O2GAS, N2GAS, CO2GAS, FURVARS, 
