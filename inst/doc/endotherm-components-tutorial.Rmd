...
This document provides an overview of an R implementation of the NicheMapR endotherm model, via the sub-functions of 'endoR' and 'endoR_devel'. This implementation modularises the original version such that many of the internal subroutines and functions can be called separately. The idea is that the user has more flexibility in setting the model up for the behavioural and physiological responses of a particular animal. All the underlying theory, equations and subroutines are described in [Endotherm Theory Tutorial](endotherm-model-equations.html). There is also a tutorial for using 'endoR' (and 'endoR_devel') itself [Endotherm Model Tutorial](endotherm-model-tutorial.html).
