...
## ----fig1, warning=FALSE, message=FALSE, echo=FALSE, fig.width=8, fig.height=5, eval=TRUE, fig.cap="**Figure 1. DEB model prediction of length through time of the lizard *Eulamprus quoyii* growing under constant, *ad libitum* food at 25 &deg;C. Photo by the author.** "----
library(NicheMapR)
...
## ----fig3, fig.width=8, fig.height=4,echo=FALSE, results='asis', fig.align='center', fig.cap='**Figure 3. Stucture of the standard model of Dynamic Energy Budget theory, from Marques et al. (2018). Boxes represent state variables, arrows represent processes.**', fig.pos='!h', message=FALSE, warnings=FALSE, eval=TRUE----
library(png)
library(grid)
img <- readPNG("DEB_scheme.png")
 grid.raster(img)

## ----fig4, fig.width=8, fig.height=4,echo=FALSE, results='asis', fig.align='center', fig.cap='**Figure 4. Stoichiometric equations of the standard model of Dynamic Energy Budget theory, taken from Kooijman (2010).**', fig.pos='!h', message=FALSE, warnings=FALSE, eval=TRUE----
library(png)
library(grid)
img <- readPNG("DEB_mass_budget.png")
 grid.raster(img)

## ----fig5, warning=FALSE, message=FALSE, echo=FALSE, fig.width=8, fig.height=5, eval=TRUE, fig.cap="**Figure 5. DEB predictions of some mass fluxes for the lizard *Eulamprus quoyii* growing under constant *ad libitum* food at 25 &deg;C. In panel a, the red line is the prediction from a general allometric relation for lizards (Andrews and Pough 1985). 'RQ' is the respiratory quotient, the ratio of $CO_2$ to $O_2$ production.**"----
par(mfrow = c(2,3))
...
knitr::kable(tail(masbal[, c(1:11)], 12))
knitr::kable(tail(masbal[, c(12:19)], 12))

## ---- echo=FALSE, results='asis', message=FALSE, warnings=FALSE----------
knitr::kable(tail(debout[, c(1:12)], 12))
knitr::kable(tail(debout[, c(13:21)], 12))

## ---- warning=FALSE, message=FALSE, eval=TRUE, fig.width=8, fig.height=5----
par(mfrow = c(1,1))
...
## ---- echo=FALSE, results='asis', message=FALSE, warnings=FALSE----------
knitr::kable(yearsout[, c(1:10)])
knitr::kable(yearsout[, c(11:22)])
knitr::kable(yearsout[, c(23:28, 34:36, 42:43)])

## ---- echo=FALSE, results='asis', message=FALSE, warnings=FALSE----------
knitr::kable(yearout[, c(1:9)])
knitr::kable(yearout[, c(10:20)])
